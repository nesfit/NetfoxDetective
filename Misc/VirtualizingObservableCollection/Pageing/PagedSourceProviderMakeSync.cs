...
namespace AlphaChiTech.Virtualization.Pageing
{
    public class PagedSourceProviderMakeSync<T> : IPagedSourceProviderAsync<T>, IProviderPreReset
    {
        ...
        public Func<T, Task<bool>> FuncContainsAsync { get; set; } = null;

        public Func<int, int, Task<PagedSourceItemsPacket<T>>> FuncGetItemsAtAsync { get; set; } = null;

        public Func<int, int, int, T> FuncGetPlaceHolder { get; set; } = null;

        public Func<Task<int>> FuncGetCountAsync { get; set; } = null;
