...
namespace VirtualizingObservableCollectionDemo.filtersort
{
    ...
    where T : IFilterOrderDescription
  {
    ...
    {
      int index = this._filterDescriptions.FindIndex( description => description.PropertyName.Equals( item.PropertyName, StringComparison.Ordinal ) );
      if (index >= 0)
      {
    ...
    {
      int index = this._filterDescriptions.FindIndex( description => description.PropertyName.Equals( propertyName, StringComparison.Ordinal ) );
      if (index >= 0)
      {
        T removed = this._filterDescriptions[index];
          this._filterDescriptions.RemoveAt( index );
