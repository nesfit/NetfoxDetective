...
namespace numl.Supervised.NeuralNetwork
{
    ...
    public class Network : IXmlSerializable
    {
        ...
        public void Forward(Vector x)
        {
            ...
            // set input
            for (int i = 0; i < this.In.Length; i++) this.In[i].Input = this.In[i].Output = i == 0 ? 1 : x[i - 1];
            // evaluate
            for (int i = 0; i < this.Out.Length; i++) this.Out[i].Evaluate();
        }
        ...
        public void Back(double t, double learningRate)
        {
            ...
            for (int i = 0; i < this.In.Length; i++) this.In[i].Error(t);

            // reset weights
            for (int i = 0; i < this.Out.Length; i++) this.Out[i].Update(learningRate);
        }
        /// <summary>
        ...
        {
            if (this._nodes == null) this._nodes = new HashSet<string>();
            else this._nodes.Clear();
        ...
        {
            if (this._edges == null) this._edges = new HashSet<Tuple<string, string>>();
            else this._edges.Clear();
