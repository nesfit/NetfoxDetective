...
namespace numl.Model
{
    ...
    public class Descriptor : IXmlSerializable
    {
        ...
        public Property this[int i]
        {
            get
            {
                if (i >= this.Features.Length) throw new IndexOutOfRangeException();
                else return this.Features[i];
            }
        }
        ...
        public int VectorLength
        {
            get
            {
                if (this._vectorLength < 0) this._vectorLength = this.Features.Select(f => f.Length).Sum();
        ...
        public IEnumerable<double> Convert(object item, bool withLabel)
        {
            ...
            if (this.Label != null && withLabel)
                foreach (double val in this.Label.Convert(Ject.Get(item, this.Label.Name)))
                    yield return val;
        ...
        public IEnumerable<IEnumerable<double>> Convert(IEnumerable<object> items)
        {
            ...
            foreach (Property feature in this.Features)
                feature.PreProcess(items);

            if (this.Label != null) this.Label.PreProcess(items);
            ...
            if (this.Label != null) this.Label.PostProcess(items);
        }
        ...
        public void ReadXml(XmlReader reader)
        {
            ...
            if (type != null && type.ToLowerInvariant() != "none") this.Type = Ject.FindType(type);
