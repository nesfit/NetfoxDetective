    ...
    public partial class Matrix : IXmlSerializable
    {
        ...
        public Matrix(int n, int d)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                for (int j = 0; j < d; j++) this._matrix[i][j] = 0;
            }
        ...
        public Matrix(double[,] m)
        {
            ...
            for (int i = 0; i < this.Rows; i++)
            {
                ...
                for (int j = 0; j < this.Cols; j++) this._matrix[i][j] = m[i, j];
            }
        }
        /// <summary>Create matrix n x n matrix.</summary>
        ...
        public Matrix(double[][] m)
        {
            this._asTransposeRef = false;
            this.Rows = m.GetLength(0);
            if (this.Rows > 0) this.Cols = m[0].Length;
            else
        ...
        public virtual Vector this[int i, VectorType t]
        {
            get
            {
                ...
                else
                {
                    ...
                    if (!this._asTransposeRef)
                    {
                        double[] cols = new double[this.Rows];
                        for (int j = 0; j < this.Rows; j++) cols[j] = this._matrix[j][i];
            ...
            set
            {
                ...
                if (t == VectorType.Row)
                {
                    ...
                    if (value.Length > this.Cols)
                        throw new InvalidOperationException(string.Format("Vector has lenght larger then {0}", this.Cols));

                    for (int k = 0; k < this.Cols; k++) this._matrix[i][k] = value[k];
                }
                else
                {
                    if (i >= this.Cols)
                    ...
                    for (int k = 0; k < this.Rows; k++) this._matrix[k][i] = value[k];
                }
            }
        }
        ...
        public Matrix this[Func<Vector, bool> f, VectorType t]
        {
            get
            {
                ...
                if (t == VectorType.Row)
                {
                    ...
                    Matrix m = new Matrix(count, this.Cols);
                    int j = -1;
                    for (int i = 0; i < this.Rows; i++)
                        if (f(this[i, t]))
                            m[++j, t] = this[i, t];
                ...
                else
                {
                    ...
                    Matrix m = new Matrix(this.Rows, count);
                    int j = -1;
                    for (int i = 0; i < this.Cols; i++)
                        if (f(this[i, t]))
                            m[++j, t] = this[i, t];
        ...
        {
            int max = t == VectorType.Row ? this.Rows : this.Cols;
            int row = t == VectorType.Row ? this.Rows - 1 : this.Rows;
            int col = t == VectorType.Col ? this.Cols - 1 : this.Cols;
        ...
        public void ReadXml(XmlReader reader)
        {
            ...
            if (this.Rows > 0 && this.Cols > 0)
            {
                ...
                for (int i = 0; i < this.Rows; i++)
                {
                    ...
                    this._matrix[i] = new double[this.Cols];
                    for (int j = 0; j < this.Cols; j++) this._matrix[i][j] = double.Parse(reader.ReadElementString("e"));
