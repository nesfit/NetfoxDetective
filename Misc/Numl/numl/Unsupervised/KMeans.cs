...
namespace numl.Unsupervised
{
    ...
    public class KMeans
    {
        ...
        public Cluster Generate(Descriptor descriptor, IEnumerable<object> examples, int k, IDistance metric = null)
        {
            ...
            for(int i = 0; i < k; i++)
                if(!this.Centers[i].IsNaN()) // check for degenerate clusters
            ...
            { 
                Id = 0, 
                Center = this.X.Mean(VectorType.Row), 
        ...
        public int[] Generate(Matrix x, int k, IDistance metric)
        {
            ...
            int[] assignments = new int[this.X.Rows];

            for (int i = 0; i < 100; i++)
            {
                // Assignment step
                Parallel.For(0, this.X.Rows, j =>
                {
                    var min_index = -1;
                ...
                // Update Step
                Matrix new_means = Matrix.Zeros(k, this.X.Cols);
        ...
        public int[] Generate(IEnumerable<object> examples, int k, IDistance metric = null)
        {
            ...
            Matrix X = this.Descriptor.Convert(examples).ToMatrix();
