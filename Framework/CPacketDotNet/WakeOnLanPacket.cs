...
#pragma warning restore 0169, 0649
        ...
        public PhysicalAddress DestinationMAC
        {
            get
            {
                ...
                Array.Copy(this.header.Bytes, this.header.Offset + syncSequence.Length, destinationMAC, 0, EthernetFields.MacAddressLength);
                return new PhysicalAddress(destinationMAC);
            }
            set
            {
                var destinationMAC = value.GetAddressBytes();
                Array.Copy(destinationMAC, 0, this.header.Bytes, this.header.Offset + syncSequence.Length, EthernetFields.MacAddressLength);
            }
        }
        #endregion
        ...
        public static WakeOnLanPacket GetEncapsulated(Packet p)
        {
            ...
            if(ethernetPacket != null) { if(ethernetPacket.Type == EthernetPacketType.WakeOnLan) { return (WakeOnLanPacket) ethernetPacket.PayloadPacket; } }
            ...
            if(udpPacket != null)
            {
                ...
                if((udpPacket.DestinationPort == 7) || (udpPacket.DestinationPort == 9)) { return (WakeOnLanPacket) udpPacket.PayloadPacket; }
            }
        ...
        public static bool IsValid(ByteArraySegment bas)
        {
            ...
            for(var i = 0; i < (EthernetFields.MacAddressLength*macRepetitions); i += EthernetFields.MacAddressLength)
            {
                ...
                if(i == 0)
                {
                    ...
                    if(!buffer.SequenceEqual(syncSequence)) { return false; }
                }
                else
                {
                    // fail the validation on malformed WOL Magic Packets
                    if(!buffer.SequenceEqual(destinationMAC)) { return false; }
                }
            }
            return true;
        }
        ...
        public override string ToString(StringOutputType outputFormat)
        {
            ...
            if(outputFormat == StringOutputType.Normal || outputFormat == StringOutputType.Colored) {
                buffer.AppendFormat("[{0}WakeOnLanPacket{1}: DestinationMAC={2}]", color, colorEscape, this.DestinationMAC);
            }
            ...
            if(outputFormat == StringOutputType.Verbose || outputFormat == StringOutputType.VerboseColored)
            {
                ...
                foreach(var property in properties) { buffer.AppendLine("WOL: " + property.Key.PadLeft(padLength) + " = " + property.Value); }
                buffer.AppendLine("WOL:");
            }
        ...
        private static readonly byte[] syncSequence = new byte[6]
        {
            0xff,
            0xff,
            0xff,
