...
namespace Netfox.SnooperHangouts.Tests
{
    class SnooperHangoutsTests : SnooperBaseTests
    {
        ...
        {
            var captureFile = this.PrepareCaptureForProcessing(SnoopersPcaps.Default.hangouts_hangouts1_pcapng);
            this.FrameworkController.ProcessCapture(captureFile);

            var conversations = this.L7Conversations.ToArray();

            this.SetPrivateKeyToConversations(conversations, PrivateKeys.Default.fb_pk);
