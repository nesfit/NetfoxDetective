...
namespace Netfox.SnooperHTTP.Tests
{
    internal class SnooperHTTPTests : SnooperBaseTests
    {
        ...
        //     public void HTTPTest_http_malyweb3()
        //     {
        //         var captureProcessor = new CaptureProcessor(SnoopersPcaps.Default.http_caps_malyweb3_pcapng);
        //         captureProcessor.TrackConversations();
        //         var conversations = captureProcessor.GetConversations("http");

        //         var exportPath = this.currentTestBaseDirectory;

        //         var frameworkController = new FrameworkController2();
        //         frameworkController.SnooperExport += this.FrameworkController_SnooperExport;
        //         var httpSnooper = this.SnooperTypes.Where(a => a.Name == "SnooperHTTP");
        //         frameworkController.ExportData(new List<Type>(httpSnooper), conversations.ToList(), exportPath);

        //         Assert.IsTrue(this.SnooperExports.Count == 1);
        //         Assert.IsTrue(this.SnooperExports.ToArray()[0].ExportObjects.Count == 2);
        ...
        //     public void Test_HTTPSleuth_with_decryption()
        //     {
        //         var pk = File.ReadAllText(PrivateKeys.Default.private_key_pem);
        //         var captureProcessor = new CaptureProcessor(SnoopersPcaps.Default.webmail_webmail_gmail_pcapng);
        //         captureProcessor.TrackConversations();
        //         var conversations = captureProcessor.GetConversations();
        //         foreach ( var conversation in conversations)
        //         {
        //             conversation.Key = new CypherKey();
        ...
        //         Assert.IsTrue(this.SnooperExports.Count == 1);
        //         Assert.IsTrue(this.SnooperExports.ToArray()[0].ExportObjects.Count == 42);
