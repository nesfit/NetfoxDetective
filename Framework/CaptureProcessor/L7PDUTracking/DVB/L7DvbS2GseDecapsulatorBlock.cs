...
namespace Netfox.Framework.CaptureProcessor.L7PDUTracking.DVB
{
    ...
    internal class L7DvbS2GseDecapsulatorBlock : ITargetBlock<L7Conversation>, ISourceBlock<PmFrameBase>
    {
        ...
        {
            return ((ITargetBlock<L7Conversation>) this._decapsulator).OfferMessage(messageHeader, messageValue, source, consumeToAccept);
        }
        #endregion

        #region Implementation of IDataflowBlock
        public void Complete() { this._decapsulator.Complete(); }

        public void Fault(Exception exception) { ((IDataflowBlock)this._decapsulator).Fault(exception); }
        ...
        {
            return ((ISourceBlock<PmFrameBase>)this._outputBuffer).ConsumeMessage(messageHeader, target, out messageConsumed);
        }
