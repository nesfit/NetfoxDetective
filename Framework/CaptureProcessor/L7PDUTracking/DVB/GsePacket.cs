...
namespace Netfox.Framework.CaptureProcessor.L7PDUTracking.DVB
{
    ...
    class GsePacket : IFragment
    {
        ...
        public static GsePacket Parse(PDUStreamReader reader)
        {
            ...
            else if(h.StartIndicator && !h.EndIndicator) { type = PacketType.Start; }
            else if(!h.StartIndicator && h.EndIndicator && h.LabelTypeIndicator == LabelType.LabelReuse)
            {
                type = PacketType.End;
            }
            else if(h.StartIndicator && h.EndIndicator) { type = PacketType.Complete; }
            else
            {
            ...
            {
                if(h.GSELength == null) { throw new InvalidPacketFormatException("GSELength field is missing."); }
    ...
    internal class GseHeader
    {
        ...
        public static GseHeader Parse(PDUStreamReader reader)
        {
            ...
            else
            {
                ...
                if(!startIndicator || !endIndicator) { fragmentID = reader.ReadByte(); }

                if(startIndicator && !endIndicator) { totalLength = reader.ReadUInt16(); }
