...
namespace Netfox.Framework.CaptureProcessor.CoreController
{
    ...
    internal class ControllerCaptureProcessorLocal : ControllerCaptureProcessorBase
    {
        ...
        public override void ProcessCapturesInternal(IEnumerable<FileInfo> captureFiles)
        {
            ...
            var storeL3StatisticsBlock   = this.CreateStoreActionBlock<L3ConversationStatistics>(out var storeL3StatisticsAwait);
            var storeL4ConversationBlock = this.CreateStoreActionBlock<L4Conversation>(out Task storeL4Await);
            var storeL4StatisticsBlock   = this.CreateStoreActionBlock<L4ConversationStatistics>(out Task storeL4StatisticsAwait);
            var storeL7ConversationBlock = this.CreateStoreActionBlock<L7Conversation>(out Task storeL7Await);
            var storeL7StatisticsBlock   = this.CreateStoreActionBlock<L7ConversationStatistics>(out Task storeL7StatisticsAwait);
            ...
            var storePmCaptureL3ConversationBlock = this.CreateStoreToJunctionActionBlock<PmCaptureL3Conversation>(out Task pmCaptureL3ConversationAwait);
            var storePmCaptureL4ConversationBlock = this.CreateStoreToJunctionActionBlock<PmCaptureL4Conversation>(out Task pmCaptureL4ConversationAwait);
            var storePmCaptureL7ConversationBlock = this.CreateStoreToJunctionActionBlock<PmCaptureL7Conversation>(out Task pmCaptureL7ConversationAwait);
            ...
            captureProcessorBlock.LinkTo(l3L4ConversationTracker, new DataflowLinkOptions() { PropagateCompletion = true });
            ...
            l7ConversationTrackerBlock.LinkTo(l7ConversationBroadcaster,         new DataflowLinkOptions() { PropagateCompletion = true });
            ...
            l7ConversationBroadcaster.LinkTo(storeL7ConversationBlock, new DataflowLinkOptions() { PropagateCompletion = true });
            L7DvbS2GseDecapsulatorBlock l7DvbS2GseDecapsulator = null;
            BufferBlock<PmFrameBase> decapsulatedFrames = null;
            if(NetfoxSettings.Default.DecapsulateGseOverUdp)
            {
                l7DvbS2GseDecapsulator = new L7DvbS2GseDecapsulatorBlock();
                decapsulatedFrames = new BufferBlock<PmFrameBase>();
                l7ConversationBroadcaster.LinkTo(l7DvbS2GseDecapsulator, new DataflowLinkOptions() { PropagateCompletion = true });
                l7DvbS2GseDecapsulator.LinkTo(decapsulatedFrames,        new DataflowLinkOptions() { PropagateCompletion = true });
            }

            foreach (var captureFile in captureFiles) { captureProcessorBlock.SendAsync(captureFile).Wait(); }
            ...
            if(!NetfoxSettings.Default.DecapsulateGseOverUdp) return;

            l7DvbS2GseDecapsulator?.Completion.Wait();
            if(decapsulatedFrames != null && decapsulatedFrames.TryReceiveAll(out var frames)) { this.ProcessFramesInternal(frames); }
        }
        ...
        private void ProcessFramesInternal([Required] IList<PmFrameBase> frames)
        {
            while(! frames.IsNullOrEmpty()){
                ...
                {
                    frame.IndexFrame(null).Wait(); // NOTE: IndexFrame called with null is insecure. Parameter `IPmCaptureProcessorBlockBase captureProcessorBlockBase`
