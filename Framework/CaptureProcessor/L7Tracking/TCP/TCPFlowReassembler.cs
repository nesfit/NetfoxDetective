...
namespace Netfox.Framework.CaptureProcessor.L7Tracking.TCP
{
    ...
    {
        public TCPFlowReassembler(L4Conversation l4Conversation, FlowStore flowStore, TimeSpan tcpSessionAliveTimeout, long tcpSessionMaxDataLooseOnTCPLoop)
        {
        ...
        public void ProcessTCPSession(FramesSequenceNumberSortedCollection tcpFlow, DaRFlowDirection flowDirection)
        {
            ...
            foreach(var synFrame in this.TCPSynOrderedFrames) { this.ProcessTcpSession(synFrame); }
            while(this.TCPSynOrderedFrames.Any()) { this.ProcessTcpSession(this.TCPSynOrderedFrames.First.Value); }
        }
        ...
        private void FiNorRstProcess()
        {
            ...
            UInt32 nextAck;
            UInt32 spuriousRetransmissions; //http://blog.packet-foo.com/2013/06/spurious-retransmissions/ example icq1.cap, frame 1056 
            ...
            {
                nextAck = ((UInt32) this.CurrentFrame.TcpSequenceNumber) + ((UInt32) this.CurrentFrame.L7PayloadLength) + 1;
                spuriousRetransmissions = ((UInt32) this.CurrentFrame.TcpSequenceNumber) + ((UInt32) this.CurrentFrame.L7PayloadLength);
            }

            this.NextFrame();
            //to consume possible rest of communication ACKs and/or retransmits 
            while(this.CurrentFrame != null
                  && (this.CurrentFrame.TcpSequenceNumber == finorRstSeq || this.CurrentFrame.TcpSequenceNumber == nextAck
                      || this.CurrentFrame.TcpSequenceNumber == spuriousRetransmissions))
            {
                this.AddNonDataFrame();
                this.NextFrame();
            }
        ...
        private void ProcessTcpSession(PmFrameBase synFrame)
        {
            ...
            {
                if(!this.TCPFlowEnumerator.MoveNext()) { throw new InvalidOperationException("Init frame does not exists!"); }
            }
            ...
            while(this.TestTCPSessionEnd(ref wasTCPSeqLoop))
            {
                ...
                if(this.CurrentPDU == null) { this.CurrentPDU = this.CurrentTCPFlow.CreateL7PDU(); }
                ...
                if(this.CurrentFrame.L7PayloadLength > 0)
                {
                    if(this.CurrentPDU.LowestTCPSeq == null) { this.CurrentPDU.LowestTCPSeq = this.CurrentFrame.TcpSequenceNumber; }
                ...
                //end of current L7PDU
                if((this.CurrentFrame.TcpFPsh || this.CurrentFrame.L7PayloadLength < this.L4Conversation.L4FlowMTU) && this.CurrentPDU.FrameList.Any()) { this.CurrentPDU = null; }

                if(this.CurrentFrame.L7PayloadLength <= 0) { this.AddNonDataFrame(); }
                this.NextFrame();
            }
        }
        ...
        private void TCPMissingSegment(Int64 x)
        {
            ...
            if(this.CurrentPDU.LowestTCPSeq == null) { this.CurrentPDU.LowestTCPSeq = this.ExpectedSequenceNumber; }
        }

        private Boolean TestTCPSessionEnd(ref Boolean wasTCPSeqLoop)
        {
            //normal state, tcpFlowEnumerator.Current == currentFrame
            if(this.TCPFlowEnumerator.Current != null && !this.TCPFlowEnumerator.Current.TcpFSyn) { return true; }
            if(this.TCPFlowEnumerator.Current != null && this.TCPFlowEnumerator.Current.TcpFSyn) { return false; }
            if(wasTCPSeqLoop) { return false; }
            wasTCPSeqLoop = true;
            ...
            while(this.TCPFlowEnumerator.Current != null && this.TCPFlowEnumerator.Current.TcpSequenceNumber - this.ExpectedSequenceNumber < this.TCPSessionMaxDataLooseOnTCPLoop)
            {
                if(this.TCPFlowEnumerator.Current.TcpSequenceNumber - this.ExpectedSequenceNumber == 0) { return true; }
                this.SkipFrame();
            }
            ...
            while(this.TCPFlowEnumerator.Current != null && this.TCPFlowEnumerator.Current.TcpSequenceNumber - this.ExpectedSequenceNumber < this.TCPSessionMaxDataLooseOnTCPLoop)
            {
                if(this.CurrentFrame.TimeStamp.Subtract(this.LastTimestamp).Duration() < this.TCPSessionAliveTimeout) { return true; }
                this.SkipFrame();
            }
            return false;
        }
