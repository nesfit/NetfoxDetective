    ...
    public class FrameworkBaseTests : UnitTestBaseSetupFixture
    {
        ...
        protected void PrintResults()
        {
            using(var dbx = this.WindsorContainer.Resolve<NetfoxDbContext>())
            {
                ...
                var allTrackedFrames = dbx.L4Conversations.Local.SelectMany(c4 => c4.Frames).Concat(dbx.L3Conversations.Local.SelectMany(c3 => c3.NonL4Frames)).ToList();
                ...
                var nonIpFrames = dbx.Frames.Local.Where(f => f.IpProtocol != IPProtocolType.IP && f.IpProtocol != IPProtocolType.IPV6).ToList();
        ...
        protected  void CheckConsistency(int framesCount, int l3ConsCount, int l4ConsCount, int l7ConsCount, int pdusCount)
        {
            using (var dbx = this.WindsorContainer.Resolve<NetfoxDbContext>())
            {
