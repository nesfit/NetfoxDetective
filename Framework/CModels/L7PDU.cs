...
{
    [Serializable]
    public class L7PDU : IExportSource
    {
        ...
        public DateTime LastSeen
        {
            get
            {
                return this._lastSeen == DateTime.MaxValue
                    ? (this._lastSeen = !this.UnorderedFrameList.IsNullOrEmpty()? this.UnorderedFrameList.Max(frame => frame.TimeStamp) : DateTime.MaxValue)
                    : this._lastSeen;
            }
        ...
        public IPEndPoint DestinationEndPoint
        {
            get
            {
                if(this.L7Conversation == null) { return null; }
        ...
        public DateTime FirstSeen
        {
            get
            {
                return this._firstSeen == DateTime.MinValue
                    ? (this._firstSeen = !this.UnorderedFrameList.IsNullOrEmpty()? this.UnorderedFrameList.Min(frame => frame.TimeStamp) : DateTime.MinValue)
                    : this._firstSeen;
            }
        ...
        {
            if(!this.UnorderedFrameList.Any()) { this.BaseFlow.AddL7PDU(this); }
