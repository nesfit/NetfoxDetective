    ...
    public class DoubleConverter
    {
        ...
        private class ArbitraryDecimal
        {
            ...
            public override String ToString()
            {
                ...
                for(var i = 0; i < this.digits.Length; i++) { digitString[i] = (char) (this.digits[i] + '0'); }

                // Simplest case - nothing after the decimal point,
                // and last real digit is non-zero, eg value=35
                if(this.decimalPoint == 0) { return new string(digitString); }

                // Fairly simple case - nothing after the decimal
                // point, but some 0s to add, eg value=350
                if(this.decimalPoint < 0) { return new string(digitString) + new string('0', -this.decimalPoint); }

                // Nothing before the decimal point, eg 0.035
                if(this.decimalPoint >= digitString.Length) { return "0." + new string('0', (this.decimalPoint - digitString.Length)) + new string(digitString); }
            ...
            internal void MultiplyBy(int amount)
            {
                ...
                if(result[0] != 0) { this.digits = result; }
                else
            ...
            {
                int first;
                for(first = 0; first < this.digits.Length; first++) { if(this.digits[first] != 0) { break; } }
                int last;
                for(last = this.digits.Length - 1; last >= 0; last--) { if(this.digits[last] != 0) { break; } }

                if(first == 0 && last == this.digits.Length - 1) { return; }

                var tmp = new byte[last - first + 1];
                for(var i = 0; i < tmp.Length; i++) { tmp[i] = this.digits[i + first]; }
