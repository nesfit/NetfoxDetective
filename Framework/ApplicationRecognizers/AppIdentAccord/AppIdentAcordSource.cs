...
namespace AppIdentAccord
{
    public class AppIdentAcordSource
    {
        ...
        public string[] Labels => this._labels ?? (this._labels = this.FeatureVectors.Select(featureVector => featureVector.ApplicationProtocolName).ToArray());
        ...
        public DecisionVariable[] DecisionVariables
        {
            get
            {
                lock(this) { return _decisionVariables ?? (this._decisionVariables = this.GetDecisionVariables()); }
            }
        }
