...
namespace Netfox.NBARDatabase
{
    ...
    {
        private static readonly ConcurrentDictionary<String, NBAR2Taxonomy> TaxonomyFilePathToTaxanomyDictionary = new ConcurrentDictionary<String, NBAR2Taxonomy>();
        ...
        public NBARProtocolPortDatabase()
        {
            ...
            if(File.Exists(NBAR2Database.DefaultPath))
            {
                taxonomyFilePath = NBAR2Database.DefaultPath;
            }
            else
            {
                var cwd = Directory.GetCurrentDirectory();
                throw new ArgumentException(string.Format("NBAR protocol port database do not exists {0}!", cwd));
            }
            ...
            this.ProtocolTaxonomyDictionary = new ConcurrentDictionary<String, NBAR2TaxonomyProtocol>(StringComparer.CurrentCultureIgnoreCase);

            lock(this._taxonomyLoaderLock)
            {
                if(taxonomyFilePath == null)
                {
                    if(TaxonomyFilePathToTaxanomyDictionary.ContainsKey("defaultFilePath")) {
                        this.Nbar2Taxonomy = TaxonomyFilePathToTaxanomyDictionary["defaultFilePath"];
                    }
                    else
                    {
                        this.Nbar2Taxonomy = NBAR2Taxonomy.Nbar2TaxonomyLoader(null);
                        TaxonomyFilePathToTaxanomyDictionary.AddOrUpdate("defaultFilePath", this.Nbar2Taxonomy, (s, taxonomy) => this.Nbar2Taxonomy = taxonomy);
                    }
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        TaxonomyFilePathToTaxanomyDictionary.AddOrUpdate(taxonomyFilePath, this.Nbar2Taxonomy, (s, taxonomy) => this.Nbar2Taxonomy = taxonomy);
                    }
                }
            }
            foreach(var protocol in this.Nbar2Taxonomy.NBAR2TaxonomyProtocol) //TODO for parallel
            {
                ...
                this.ProtocolTaxonomyDictionary.AddOrUpdate(protocol.name, u => protocol,
                    (u, proto) => throw new ArgumentException("Key already exists, check NBAR taxonomy database file", "protocol"));
            }
        ...
        public NBAR2TaxonomyProtocol GetNbar2TaxonomyProtocol(String nbarProtocolName)
        {
            ...
            if(nbarProtocolName == null) { return null; }
