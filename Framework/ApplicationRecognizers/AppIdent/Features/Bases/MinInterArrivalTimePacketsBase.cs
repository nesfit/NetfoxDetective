...
{
    public class MinInterArrivalTimePacketsBase : FeatureBase
    {
        public MinInterArrivalTimePacketsBase() { }
        public MinInterArrivalTimePacketsBase(L7Conversation l7Conversation, DaRFlowDirection flowDirection) : base(l7Conversation, flowDirection) { }
        public MinInterArrivalTimePacketsBase(double featureValue) : base(featureValue) { }
        ...
        public override double ComputeFeature(L7Conversation l7Conversation, DaRFlowDirection flowDirection)
        {
            ...
            var minTime = (pmFrameBases[1].FirstSeen - pmFrameBases[0].FirstSeen).TotalSeconds;

            for(var i = 1; i < length - 1; i++)
            {
                var interArrivalTime = (pmFrameBases[i + 1].FirstSeen - pmFrameBases[i].FirstSeen).TotalSeconds;
