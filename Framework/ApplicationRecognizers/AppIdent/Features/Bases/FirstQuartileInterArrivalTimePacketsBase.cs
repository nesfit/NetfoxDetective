...
{
    public class FirstQuartileInterArrivalTimePacketsBase : FeatureBase
    {
        public FirstQuartileInterArrivalTimePacketsBase() { }
        public FirstQuartileInterArrivalTimePacketsBase(L7Conversation l7Conversation, DaRFlowDirection flowDirection) : base(l7Conversation, flowDirection) { }
        public FirstQuartileInterArrivalTimePacketsBase(double featureValue) : base(featureValue) { }
        ...
        public override double ComputeFeature(L7Conversation l7Conversation, DaRFlowDirection flowDirection)
        {
            ...
            for(var i = 0; i < length - 1; i++) { times[i] = Math.Abs((pmFrameBases[i + 1].FirstSeen - pmFrameBases[i].FirstSeen).TotalSeconds); }

            int medianIndex;
            if(times.Length == 1) { medianIndex = 1; }
            else { medianIndex = times.Length / 2; }
            Array.Sort(times);
            return GetMedian(times.Take(medianIndex).ToArray());
        }
