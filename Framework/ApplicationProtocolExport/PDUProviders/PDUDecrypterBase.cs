...
namespace Netfox.Framework.ApplicationProtocolExport.PDUProviders
{
    ...
    {
        public bool Equals(Byte[] a, Byte[] b) { return a.SequenceEqual(b); }
    ...
    public class PDUDecrypterBase : PDUStreamBasedProvider
    {
        ...
        private static readonly Dictionary<Byte[], Byte[]> SessionsMasterKeys = new Dictionary<Byte[], Byte[]>(new ByteArrayComparer());
        ...
        public PDUDecrypterBase(L7Conversation conversation, EfcPDUProviderType pduProviderType) : base(conversation, pduProviderType)
        {
                ...
                case SeekOrigin.Current:
                    if(offset < 0)
                    {
                        ...
                        while(-offset > seeked)
                        {
                            ...
                            {
                                seeked += (this.Current.DataOffset == 0 && this._decryptedDataCnt != 0)? this.Current.DataLength : this.Current.DataOffset;
        ...
        {
            if(this._dataDecrypted == false) { this.DecryptPdus(); }
        ...
        public override L7PDU GetCurrentPDU() => this._decryptedDataCnt < this.DataList.Count? this.DataList.ElementAt(this._decryptedDataCnt).PDU : null;
        ...
        {
            if(this._dataDecrypted == false) { this.DecryptPdus(); }
            if(!this.DataList.Any()) { return false; }

            if(this.Current.DataOffset != 0 || this._decryptedDataCnt == 0) { this.DataList.RemoveAt(this._decryptedDataCnt); }

            for(var i = this._decryptedDataCnt - 1; i >= 0; i--)
            {
                var decryptedData = this.DataList[i];
                if(decryptedData.IsAllRead) { this.DataList.RemoveAt(i); }
            }
        ...
        {
            if(this._decryptedDataCnt <= 0) { return false; }
        ...
        private Boolean NextContinueInterlay(ref Int32 indexpdu)
        {
            ...
            {
                indexpdu++;
                if(this.DataList.ElementAt(0).PDU.FlowDirection == this.DataList.ElementAt(indexpdu).PDU.FlowDirection) return true;
            }
