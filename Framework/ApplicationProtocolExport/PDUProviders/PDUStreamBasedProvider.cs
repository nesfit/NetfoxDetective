...
// file:	PDUStreamBasedProvider.cs
//
// summary:	Implements the PDU stream base class
...
namespace Netfox.Framework.ApplicationProtocolExport.PDUProviders
{
    ...
    public class PDUStreamBasedProvider : PDUDataProvider, IPduStreamBasedProvider
    {
        ...
        public override Boolean NewMessage()
        {
            ...
            if(this.Current.Offset != 0 || this._currentPDUindex == 0) { this._conversationPDUs.RemoveAt(this._currentPDUindex); }
            ...
            for(var i = this._currentPDUindex - 1; i >= 0; i--)
            {
                ...
                if(pdu.IsAllRead) { this._conversationPDUs.RemoveAt(i); }
            }
            return this.Reset();
        }
                ...
                case SeekOrigin.Current:
                    if(offset < 0)
                    {
                        ...
                        while(-offset > seeked)
                        {
                            ...
                            {
                                seeked += (this.Current.Offset == 0 && this._currentPDUindex != 0)? this.Current.Length : this.Current.Offset;
                                if(!this.MovePrevious()) { return this.Position; }
                            }
                            else
                            {
        ...
        public override Int32 Read(Byte[] buffer, Int32 offset, Int32 requestedCount)
        {
            ...
            do
            {
                ...
                if(this.Current.RemainingBytes == 0) { this.EndOfPDU = true; }
                if(count != 0) { this.EndOfStream = !this.MoveNext(); }
        ...
        public override Int32 Peek(Byte[] buffer, Int32 bufferOffset, Int32 streamPosition, Int32 requestedCount, SeekOrigin origin)
        {
            if(!this._conversationPDUs.Any()) { return 0; }

            if(origin != SeekOrigin.Current || streamPosition < 0) { throw new NotImplementedException("Not implemented yet, request implementation."); }
            ...
            while(streamPosition > 0)
            {
                if(streamPosition > this.CurrentPeek.Length)
                {
                    ...
                    if(!this.PeekNext()) { return (Int32) offset - bufferOffset; }
                }
                else
                {
                    break;
            ...
            do
            {
                ...
                count -= currentCount;
                streamPosition = 0;
                if(count != 0) { successfulUpdate = this.PeekNext(); }
        ...
        {
            if(index < 0 || index >= this._conversationPDUs.Count) { throw new InvalidOperationException(); }
        ...
        {
            foreach(var conversationPdU in this._conversationPDUs) //TODO maybe it would be enough to reset only PDUs with index < currentPDUindex
            {
        ...
        {
            if(this._currentPDUindex <= 0) { return false; }
        ...
        private Boolean NextMixed(ref Int32 indexPDU)
        {
            ...
            if(indexPDU >= this._conversationPDUs.Count - 1) { return false; }
        ...
        private Boolean NextContinueInterlay(ref Int32 indexpdu)
        {
            ...
            {
                indexpdu++;
                if(this.Value(0).Pdu.FlowDirection == this.Current.Pdu.FlowDirection) { return true; }
            }
        ...
        public override L7PDU GetCurrentPDU() { return this._conversationPDUs.Count == 0? null : this._conversationPDUs[this._currentPDUindex].Pdu; }
        
        
        #endregion

        public IEnumerable<PmFrameBase> ProcessedFrames => this._conversationPDUs.Take(this._currentPDUindex).SelectMany(p => p.Pdu.FrameList);
