...
namespace Netfox.Framework.ApplicationProtocolExport.PDUProviders
{
    public class PDUDecrypter
    {
        ...
        public Boolean DoDecryption(Byte[] pdu, PmFrameBase frame, ref Byte[] decryptedBytes)
        {
            ...
            if(Enumerable.SequenceEqual(source, this.ClientDirection)) { this.DataDecrypter.DecreaseClientSeq(); }
            else
        ...
        private Boolean tls_hmac_check(Int32 sequenceNum, HMac hmac, Byte[] ct, Byte[] version, Int32 msgLen, Byte[] msg, Byte[] recvDigest)
        {
            var seq = BitConverter.GetBytes(sequenceNum);
            var exSeq = new Byte[8];
            if(BitConverter.IsLittleEndian) { Array.Reverse(seq); }
