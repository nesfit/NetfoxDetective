...
namespace Netfox.Detective.ViewModels
{
    public class ApplicationShell : DetectiveWindowViewModelBase, ISystemComponent, IApplicationShell
    {
        ...
        public ConcurrentObservableCollection<DetectiveWindowViewModelBase> Windows { get; } = new ConcurrentObservableCollection<DetectiveWindowViewModelBase>();
        ...
        //[IgnoreAutoChangeNotification]
        //public Type[] AvailableAnalyzerTypes => this.ApplicationOrInvestigationWindsorContainer.Kernel.GetHandlers(typeof(IAnalyzer))
        //    .Concat(this.ApplicationOrInvestigationWindsorContainer.Kernel.GetHandlers(typeof(IAnalyzerInvestigation)))
        //    .Concat(this.ApplicationOrInvestigationWindsorContainer.Kernel.GetHandlers(typeof(IAnalyzerApplication)))
        ...
        [IgnoreAutoChangeNotification]
        public ICommand CShowWorkspaceManager => new RelayCommand(() => this.NavigationService.Show(typeof(WorkspacesManagerVm)));

        [IgnoreAutoChangeNotification]
        public ICommand CShowInvestigationManager => new RelayCommand(() => this.NavigationService.Show(typeof(InvestigationsManagerVm)));

        [IgnoreAutoChangeNotification]
        public ICommand CShowCaptureOverview => new RelayCommand(() => this.NavigationService.Show(typeof(ConversationsOverviewVm)));

        [IgnoreAutoChangeNotification]
        public ICommand CShowConversationsGroupOverview => new RelayCommand(() => this.Logger?.Error($"Action is not implemented {Environment.StackTrace}"));

        [IgnoreAutoChangeNotification]
        public ICommand CShowConversationsGroupContent => new RelayCommand(() => this.Logger?.Error($"Action is not implemented {Environment.StackTrace}"));

        [IgnoreAutoChangeNotification]
        public ICommand CShowConversation => new RelayCommand(() => this.Logger?.Error($"Action is not implemented {Environment.StackTrace}"));

        //[IgnoreAutoChangeNotification]
        //public ICommand CShowExportOverview => new RelayCommand(() => this.NavigationService.Show(typeof(ExportOverviewVm)));
        [IgnoreAutoChangeNotification]
        public ICommand CShowExportContentExplorer => new RelayCommand(() => this.NavigationService.Show(typeof(ExportContentExplorerVm)));

        [IgnoreAutoChangeNotification]
        public ICommand CShowSelectiveExport => new RelayCommand(() => this.NavigationService.Show(typeof(SelectiveExportVm)));

        [IgnoreAutoChangeNotification]
        public ICommand CShowFullTextSearch => new RelayCommand(() => this.Logger?.Error($"Action is not implemented {Environment.StackTrace}"));
        ...
        [IgnoreAutoChangeNotification]
        public RelayCommand CShowTaskManager => this._cShowTaskManager ?? (this._cShowTaskManager = new RelayCommand(() => this.NavigationService.Show(typeof(BgTasksManagerVm))));
