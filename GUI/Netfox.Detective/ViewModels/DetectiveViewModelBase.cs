...
{
    public abstract class DetectiveViewModelBase : ViewModelBase, IDisposable, ISystemComponent, INotifyPropertyChanged, ILoggable
    {
        ...
        [IgnoreAutoChangeNotification]
        public ISystemServices SystemServices => new SystemServices();

        [IgnoreAutoChangeNotification]
        public NavigationService NavigationService => this._navigationService ?? (this._navigationService = this.ApplicationOrInvestigationWindsorContainer.Resolve<NavigationService>());

        [IgnoreAutoChangeNotification]
        public BgTasksManagerService BgTasksManagerService
            => this._bgTasksManagerService ?? (this._bgTasksManagerService = this.ApplicationOrInvestigationWindsorContainer.Resolve<BgTasksManagerService>());

        [IgnoreAutoChangeNotification]
        public BgTasksManagerVm BgTasksManagerVm => this._bgTasksManagerVm ?? (this._bgTasksManagerVm = this.ApplicationOrInvestigationWindsorContainer.Resolve<BgTasksManagerVm>());

        [IgnoreAutoChangeNotification]
        public IWindsorContainer ApplicationOrInvestigationWindsorContainer { get; protected set; }

        [IgnoreAutoChangeNotification]
        public ApplicationShell ApplicationShell => this._applicationShell ?? (this._applicationShell = this.ApplicationOrInvestigationWindsorContainer.Resolve<ApplicationShell>());
