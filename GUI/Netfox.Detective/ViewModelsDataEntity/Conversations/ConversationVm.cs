...
namespace Netfox.Detective.ViewModelsDataEntity.Conversations
{
    public class ConversationVm : DetectiveDataEntityViewModelBase, IDataEntityVm
    {
        ...
        public RelayCommand CShowReassembledStrem => new RelayCommand(() => this.NavigationService.Show(typeof(ReassembledStreamDetailVm), this, true));
        ...
        public ViewModelVirtualizingIoCObservableCollection<FrameVm, PmFrameBase> Frames
            =>
                this._Frames
                ?? (this._Frames =
                    new ViewModelVirtualizingIoCObservableCollection<FrameVm, PmFrameBase>(this.Conversation.Frames.ToList(), this.ApplicationOrInvestigationWindsorContainer));
        ...
        public FrameVm CurrentPacket
        {
            ...
            set
            {
                if(this._currentPacket == value) { return; }
        ...
        private L7ConversationStatistics L7ConversationStatistics => (this.Conversation.ConversationStats as L7ConversationStatistics);
        ...
        public object MissingBytesPerc
        {
            get
            {
                ...
                if(extractedBytes != null && extractedBytes != 0) { return this.MissingBytes/extractedBytes; }
                return 0;
            }
        }
        #endregion
        ...
        private IEnumerable CreateTrafficHistory(IEnumerable<FrameVm> frames, bool revert)
        {
            ...
            foreach(var p in frames) { if(p != null) { yield return new KeyValue<DateTime, long>(p.TimeStamp, (revert? -p.Frame.OriginalLength : p.Frame.OriginalLength), p); } }
        }

        public IEnumerable TrafficHistoryUp => this.CreateTrafficHistory(this.Frames.Where(f=>this.DeterminFrameDirection(f.Frame)==DaRFlowDirection.up), false);


        public IEnumerable TrafficHistoryDown => this.CreateTrafficHistory(this.Frames.Where(f => this.DeterminFrameDirection(f.Frame) == DaRFlowDirection.down), true);
        ...
        public IEnumerable MissingDataHistory
        {
            get
            {
                foreach (var frameVm in this.Frames.Where(f => f.Frame is PmFrameVirtual))
                {
                    yield return new ConversationMissingData(){TimeStamp = frameVm.TimeStamp, Length = frameVm.FrameSize};
                }
        ...
        public class ConversationMissingData
        {
            ...
            public DateTime TimeStamp
            {
                ...
                set
                {
                    this._timeStamp = DateTime.SpecifyKind(value, DateTimeKind.Utc);
                }
            }
        ...
        public QualityFrameVm SelectedQualityFrame
        {
            ...
            set
            {
                ...
                if(value != null)
                {
                    this.CurrentPacket = this.ApplicationOrInvestigationWindsorContainer.Resolve<FrameVm>(new
                    {
                        model = value.Frame,
        ...
        public IEnumerable<QualityFrameVm> FramesQualityUp
        {
            get
            {
                ...
                if(l7conv == null || l7stats == null) { return null; }
        ...
        public EncodingInfo L7PlaintextEncoding
        {
            set
            {
                if(value != null) { this._l7PlaintextEncoding = value.GetEncoding(); }
        ...
        public IEnumerable<ReassembledStreamPduVm> ReassembledStream
        {
            ...
            {
                var l7conv = this.Conversation as L7Conversation;
                var reassmbledReassembledStream = new ReassembledStreamPduVm[l7conv.L7PDUs.Count()];

                Parallel.ForEach(l7conv.L7PDUs,
                    (L7PDU, state, index) =>
                    {
                        reassmbledReassembledStream[index] = new ReassembledStreamPduVm(this.ApplicationOrInvestigationWindsorContainer, L7PDU, this._l7PlaintextEncoding);
                    });

                return reassmbledReassembledStream;
            }
        }
