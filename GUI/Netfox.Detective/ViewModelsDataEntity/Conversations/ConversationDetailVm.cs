...
namespace Netfox.Detective.ViewModelsDataEntity.Conversations
{
    ...
    {
        public ConversationDetailVm(WindsorContainer applicationWindsorContainer, ConversationVm model) : base(applicationWindsorContainer, model)
        {
            this.ConversationVm = model;
            DispatcherHelper.CheckBeginInvokeOnUI(() => this.View = this.ApplicationOrInvestigationWindsorContainer.Resolve<IConversationDetailView>());
        }
