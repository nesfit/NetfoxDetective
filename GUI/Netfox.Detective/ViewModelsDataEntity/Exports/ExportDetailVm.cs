...
namespace Netfox.Detective.ViewModelsDataEntity.Exports
{
    public class ExportDetailVm : DetectiveIvestigationDataEntityPaneViewModelBase
    {
        ...
        private void ScanAssembliesForExportDetailPanesAndRegisterThem()
        {
            ...
            {
                var exportDetailVmTypes =
                    currentAssembly.GetTypes()
                        .Where(a => a.IsClass && a.IsPublic && !a.IsInterface && !a.IsAbstract && typeof(DetectiveExportDetailPaneViewModelBase).IsAssignableFrom(a));
                ...
                foreach(var vmType in exportDetailVmTypes)
                {
                    try
                    {
                        ...
                        {
                            try {
                                this.ApplicationOrInvestigationWindsorContainer.Register(Component.For(vmType).LifestyleTransient());
                            }
                            catch(Exception ex) {
