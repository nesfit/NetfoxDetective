...
namespace Netfox.Detective.ViewModelsDataEntity.Exports.Detail
{
    ...
    public class VoIPCallDetailVm : DetectiveExportDetailPaneViewModelBase
    {
        ...
        public VoIPCallDetailVm(WindsorContainer applicationWindsorContainer, ExportVm model, IVoIPCallDetailView view) : base(applicationWindsorContainer, model, view)
        {
            try
            {
                ...
                this.ExportVmObserver.RegisterHandler(p => p.SelectedSnooperExportObject, p =>
                {
                    if(p.SelectedSnooperExportObject is ICall)
                    {
                        try
                        {
                            ...
                            this.CallStreams = this.SelectedCall.CallStreams.Select(callStream => new CallStreamVm(callStream)).ToArray();
        ...
        {
            foreach(var call in this.CallStreams) { call.Play(); }
            foreach(var call in this.PossibleCallStreams) { call.Play(); }
            this.CPlay.RaiseCanExecuteChanged();
            this.CPause.RaiseCanExecuteChanged();
            this.CStop.RaiseCanExecuteChanged();
        }, () =>
        {
            ...
            if(canCallStreams.HasValue && canCallStreams.Value) { return true; }
            if(canPossibleCallStreams.HasValue && canPossibleCallStreams.Value) { return true; }
            return false;
        }));
        ...
        {
            foreach(var call in this.CallStreams) { call.Pause(); }
            foreach(var call in this.PossibleCallStreams) { call.Pause(); }
            this.CPlay.RaiseCanExecuteChanged();
            this.CPause.RaiseCanExecuteChanged();
            this.CStop.RaiseCanExecuteChanged();
        }, () =>
        ...
        {
            foreach(var call in this.CallStreams) { call.Stop(); }
            foreach(var call in this.PossibleCallStreams) { call.Stop(); }
            this.CPlay.RaiseCanExecuteChanged();
            this.CPause.RaiseCanExecuteChanged();
            this.CStop.RaiseCanExecuteChanged();
        }, () =>
        ...
        {
            var callStreams =
                this.ApplicationOrInvestigationWindsorContainer.Resolve<VirtualizingObservableDBSetPagedCollection<SnooperExportBase>>()
                    .SelectMany(export => export.ExportObjects.Where(eobj => eobj is ICallStream))
                    .Where(
                        stream =>
                            this.SelectedCall.CallStreamAddresses.Contains(stream.SourceEndPoint) && this.SelectedCall.CallStreamAddresses.Contains(stream.DestinationEndPoint));
            foreach(var callStream in callStreams)
            {
                if(this.SelectedCall.CallStreams.Contains(callStream as ICallStream)) { return; }
            ...
            {
                if(this.SelectedCall.PossibleCallStreams.Contains(possibleCallStream as ICallStream)) { return; }
