...
namespace Netfox.Detective.Services
{
    public class ExportService : DetectiveInvestigationService
    {
        ...
        public BgTasksManagerService BgTasksManagerService
            => this._bgTasksManagerService ?? (this._bgTasksManagerService = this.InvestigationWindsorContainer.Resolve<BgTasksManagerService>());
            ...
            bool dontUseApplicationTag)
        {
            ...
            if(exporters == null) { throw new ArgumentException(@"exporters cannot be null", nameof(exporters)); }
            await Task.Run(() =>
            {
                var lxConversations = conversations as ILxConversation[] ?? conversations.ToArray();
                this.Logger?.Error($"Starting export application data: {lxConversations.Count()} conversations to group {exportGroupName}");
                this.ExportDataAction(conversations,null,exporters,exportGroupName,createSubGroup,dontUseApplicationTag);
            });
        }
            ...
            bool showExportedObjects)
        {
            ...
            await Task.Run(() =>
            {
                ...
                var i = 0;
                foreach (var sourceLog in sourceLogs) { logs[i++] = sourceLog; } //to overcome ToArray BUG
            ...
            
        {
            try
            {
                ...
                var conversationSnoopers = exporters.Where(s => s.GetType().GetConstructors().Any(c => c.GetParameters().Any(p => p.ParameterType == typeof(SelectedConversations))));
                var exportSnoopers = exporters.Where(s => s.GetType().GetConstructors().Any(c => c.GetParameters().Any(p => p.ParameterType == typeof(IEnumerable<SnooperExportBase>))));
                var sourceLogSnoopers = exporters.Where(s => s.GetType().GetConstructors().Any(c => c.GetParameters().Any(p => p.ParameterType == typeof(IEnumerable<FileInfo>))));
                ...
                this.FrameworkController.ExportData(exportSnoopers, exports, this.InvestigationInfo.ExportsDirectoryInfo);
