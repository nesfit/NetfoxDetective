...
{
    public abstract class ConversationsVm<TType> : DetectiveDataEntityViewModelBase, IDataEntityVm, IConversationsVm
        where TType : IConversationsModel
    {
        ...
        protected ConversationsVm(IWindsorContainer applicationWindsorContainer, TType model,
            ExportService exportService) : base(applicationWindsorContainer, model)
        {
            ...
            this.Frames =
                new ViewModelVirtualizingIoCObservableCollection<FrameVm, PmFrameBase>(this.Model.Frames,
                    this.InvestigationWindsorContainer);
            this.L3Conversations =
                new ViewModelVirtualizingIoCObservableCollection<ConversationVm, L3Conversation>(
                    this.Model.L3Conversations, this.InvestigationWindsorContainer);
            this.L4Conversations =
                new ViewModelVirtualizingIoCObservableCollection<ConversationVm, L4Conversation>(
                    this.Model.L4Conversations, this.InvestigationWindsorContainer);
            this.L7Conversations =
                new ViewModelVirtualizingIoCObservableCollection<ConversationVm, L7Conversation>(
                    this.Model.L7Conversations, this.InvestigationWindsorContainer);

            this.UsedSnoopers =
                new ViewModelVirtualizingIoCObservableCollection<SnooperVm, ISnooper>(this.Model.UsedSnoopers,
                    this.InvestigationWindsorContainer);
        ...
        public IWindsorContainer
            InvestigationWindsorContainer { get; private set; } //never change to public!!! .NET BUG!!!
        ...
        public FrameVm CurrentFrame
        {
            ...
            set
            {
                ...
                if (value != null && !this.Frames.Contains(value))
                {
                    return;
                }
        ...
        public RelayCommandAsync<List<object>> CActualizeExportSet
            => new RelayCommandAsync<List<object>>(
                async (list, token) => await Task.Run(async () => { await this.ActualizeExportSet(list); }, token), o => true);
        ...
        public InvestigationVm InvestigationVm => this._investigationVm ??
                                                  (this._investigationVm = this.InvestigationWindsorContainer
                                                      .Resolve<InvestigationVm>());

        // TODO: implement
        public RelayCommand CConversationChartDClick => new RelayCommand(() =>
            this.Logger?.Error($"Action is not implemented {Environment.StackTrace}"));

        public RelayCommand CAddCypherKeyFromFile => new RelayCommand(() =>
        {
            var pkFilePath =
                this.SystemServices.OpenFileDialog("", ".pem", "Private key (*.pem)|*.pem;|All files (*.*)|*.*");

            if (string.IsNullOrEmpty(pkFilePath))
            {
                return;
            }
            ...
            get
            {
                if (this._conversationsStatisticsVm == null)
                {
                    ...
                    this._conversationsStatisticsVm = new NotifyTaskCompletion<ConversationsStatisticsVm>(async () =>
                        ...
                        }, () => { this.OnPropertyChanged(nameof(this.ConversationsStatisticsVm)); },
                        conversationStatistics, false);
                }
                ...
                if (this._conversationsByDuration == null)
                {
                    this._conversationsByDuration = new NotifyTaskCompletion<ConversationVm[]>(async () =>
                    {
                        return await Task.Run(() =>
                        {
                            ...
                            Array.Sort(conversationsByDuration,
                                (t1, t2) => -t1.Conversation.ConversationStats.Duration.CompareTo(t2.Conversation
                ...
                if (this._conversationsByTraffic == null)
                {
                    this._conversationsByTraffic = new NotifyTaskCompletion<ConversationVm[]>(async () =>
                    {
                        return await Task.Run(() =>
                        {
                            ...
                            Array.Sort(conversationsByTraffic,
                                (t1, t2) => -t1.Conversation.ConversationStats.Bytes.CompareTo(t2.Conversation
                                    .ConversationStats.Bytes));
        ...
        public ConversationsStatisticsVm ConversationsStatisticsVm =>
            this._ConversationsStatisticsVm; //necessary for NotifyTaskCompletion

        public RelayCommand<ConversationVm> CShowConversationDetail
            =>
                this._cShowConversationDetail
                ?? (this._cShowConversationDetail = new RelayCommand<ConversationVm>(conv =>
                    this.NavigationService.Show(typeof(ConversationDetailVm), conv)));
        ...
        public ConversationVm CurrentConversation
        {
            ...
            set
            {
                if (this._currentConversation == value)
                {
                    return;
                }

                if (value != null && !(this.L3Conversations.Contains(value) || this.L4Conversations.Contains(value) ||
                                       this.L7Conversations.Contains(value)))
                {
                    return;
                }
        ...
        public ViewModelVirtualizingIoCObservableCollection<FrameVm, PmFrameBase> Frames
        {
            get
            {
                return this._frames ?? (this._frames =
                    new ViewModelVirtualizingIoCObservableCollection<FrameVm, PmFrameBase>(this.Model.Frames,
                        this.InvestigationWindsorContainer));
            }
        ...
        public void CreateConversationGroup(IList conversationsVmList)
        {
            ...
            if (conversationsVmList.Count == 0)
            {
                this.Logger?.Error($"No conversations selected");
                return;
            }

            var conversationGroups = this.InvestigationWindsorContainer
                .Resolve<VirtualizingObservableDBSetPagedCollection<ConversationsGroup>>();
        ...
        protected virtual async Task ActualizeExportSet(List<object> parameters)
        {
            ...
            await this.ExportService.Export(this.Model.L7Conversations, this.CurrentInvestigationSelectedExporters(),
                null, false, showExportedObjectsDuringExportation,
                dontUseApplicationTagsForceOnAllConversations);
        }

        protected List<ISnooper> CurrentInvestigationSelectedExporters()
        {
            return AvailableSnoopers.Where(i => i.IsEnabled).Select(i => i.Snooper).ToList();
        }
        ...
        private ConversationVm[] GetFilteredL7ConversationsArray()
        {
            if (this._filteredL7ConversationsArray == null)
            {
                lock (this._getFilteredL7ConversationsArrayLock)
                {
                    ...
                    var avgDur = (long) this.Average(this.L7Conversations,
                        vm => (long) vm.Conversation.ConversationStats.Duration.TotalMilliseconds);
                    var avgTraff = (long) this.Average(this.L7Conversations,
                        vm => vm.Conversation.ConversationStats.Bytes);
                    Parallel.ForEach(this.L7Conversations, (conversation, state, index) =>
                    {
                        if (avgDur > conversation.Conversation.ConversationStats.Duration.TotalMilliseconds &&
                            avgTraff > conversation.Conversation.ConversationStats.Bytes)
                        {
                            return;
                        }
        ...
        private void UsedSnoopersOnCollectionChanged(object sender,
