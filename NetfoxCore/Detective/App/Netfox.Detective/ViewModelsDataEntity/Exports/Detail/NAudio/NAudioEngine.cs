...
using WPFSoundVisualizationLib.Spectrum_Analyzer;
    ...
    public class NAudioEngine : INotifyPropertyChanged, ISpectrumPlayer, IWaveformPlayer, IDisposable
    {
        ...
        public double ChannelLength
        {
            ...
            protected set
            {
                ...
                if (oldValue != this.channelLength)
                {
        ...
        public double ChannelPosition
        {
            ...
            set
            {
                if (!this.inChannelSet)
                {
                    ...
                    {
                        this.ActiveStream.Position = (long) ((position / this.ActiveStream.TotalTime.TotalSeconds) *
                                                             this.ActiveStream.Length);
                    }

                    this.channelPosition = position;
                    if (oldValue != this.channelPosition)
                    {
        ...
        private void GenerateWaveformData(string path)
        {
            ...
            {
                this.waveformGenerateWorker.RunWorkerAsync(new WaveformGenerationParams(waveformCompressedPointCount,
                    path));
            }
        }
        ...
        private void waveformGenerateWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            if (e.Cancelled)
            {
                if (!this.waveformGenerateWorker.IsBusy && waveformCompressedPointCount != 0)
                {
                    this.waveformGenerateWorker.RunWorkerAsync(
                        new WaveformGenerationParams(waveformCompressedPointCount, this.pendingWaveformPath));
                }
            }
        }
        ...
        private void inputStream_Sample(object sender, SampleEventArgs e)
        {
            ...
            var repeatStartPosition =
                (long) ((this.SelectionBegin.TotalSeconds / this.ActiveStream.TotalTime.TotalSeconds) *
                        this.ActiveStream.Length);
            var repeatStopPosition =
                (long) ((this.SelectionEnd.TotalSeconds / this.ActiveStream.TotalTime.TotalSeconds) *
                        this.ActiveStream.Length);
            if (((this.SelectionEnd - this.SelectionBegin) >= TimeSpan.FromMilliseconds(repeatThreshold)) &&
                this.ActiveStream.Position >= repeatStopPosition)
            {
        ...
        void positionTimer_Tick(object sender, EventArgs e)
        {
            if (this.ActiveStream != null)
            {
                ...
                this.ChannelPosition = (this.ActiveStream.Position / (double) this.ActiveStream.Length) *
                                       this.ActiveStream.TotalTime.TotalSeconds;
