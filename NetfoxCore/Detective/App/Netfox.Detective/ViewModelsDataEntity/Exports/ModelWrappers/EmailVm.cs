...
namespace Netfox.Detective.ViewModelsDataEntity.Exports.ModelWrappers
{
    ...
    public class EmailVm : DetectiveDataEntityViewModelBase
    {
        ...
        public EmailVm(WindsorContainer applicationWindsorContainer, object model, ExportVm exportVm) : base(
        ...
        public DateTime TimeStamp
        {
            get
            {
                ...
                if (DateTime.TryParseExact(this.EmailRoot.MIMEpart.Date, "yyyy-MM-dd hh.mm.ss.fffffff",
        ...
        {
            get { return this.Attachments.Any(); }
        }
        ...
        public string Description
        {
            get
            {
                ...
                if (this.Email.EMailType == EMailType.RawEmail)
                {
                    return defaultDescription;
                }

                var mimeEmail = (MIMEemail) this.Email;
                if (mimeEmail.DocumentRoot == null)
                {
                    return defaultDescription;
                }
                ...
                if (String.IsNullOrEmpty(from) && this.Email.SourceEndPoint != null)
                {
                    from = this.Email.SourceEndPoint.ToString();
                }

                if (String.IsNullOrEmpty(to) && this.Email.DestinationEndPoint != null)
                {
                    to = this.Email.DestinationEndPoint.ToString();
                }
        ...
        public string ShortPlainBody
        {
            get
            {
                ...
                if (body == null)
                {
                    ...
                    if (shortPlainBody.Length > 150)
                    {
                        shortPlainBody = shortPlainBody.Substring(0, 150) + " ...";
                    }
                }
                else
                {
        ...
        public string SelectedPartContentHeader =>
            this.SelectedPartHeader + Environment.NewLine + this.SelectedPartContent;
        ...
        public IEnumerable<MimePartVm> Bodies
        {
            get
            {
                ...
                {
                    if (!String.IsNullOrEmpty(emailPart.MIMEpart.ContentType) && !String.IsNullOrEmpty(
                                                                                  emailPart.MIMEpart.ContentSubtype)
                                                                              && (emailPart.MIMEpart.ContentType.Equals(
                                                                                  "text") && (emailPart.MIMEpart
                                                                                      .ContentSubtype.Equals("plain") ||
                                                                                  emailPart.MIMEpart.ContentSubtype
                                                                                      .Equals(
                                                                                          "html"))))
                    {
            ...
            get
            {
                foreach (var emailPart in this.FlatEmail)
                {
                    if (!String.IsNullOrEmpty(emailPart.MIMEpart.ContentDisposition)
                        && (emailPart.MIMEpart.ContentDisposition.Equals("attachment") ||
                            emailPart.MIMEpart.ContentDisposition.Equals("inline")))
                    {
                        yield return emailPart;
                    }
                }
