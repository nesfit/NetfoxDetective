...
namespace Netfox.Detective.ViewModelsDataEntity.Exports
{
    ...
    public class ExportGroupVm : DetectiveDataEntityViewModelBase, IDataEntityVm
    {
        ...
        public ExportGroupVm(IWindsorContainer applicationWindsorContainer, ExportGroup model) : base(
            applicationWindsorContainer, model)
        {
            this.ExportGroup = model;
            this.Exports =
                new ViewModelVirtualizingIoCObservableCollection<ExportVm, SnooperExportBase>(this.ExportGroup.Exports,
                    applicationWindsorContainer);
            this.ExportGroups =
                new ViewModelVirtualizingIoCObservableCollection<ExportGroupVm, ExportGroup>(
                    this.ExportGroup.ExportGroups, applicationWindsorContainer);
            ...
            get
            {
                lock (this.Exports.SyncRoot)
                {
                    return this._allExport
                           ?? (this._allExport =
                               new NotifyTaskCompletion<ExportVm[]>(
                                   () => Task.Run(() => this.RecursiveExportResults(this).ToArray()),
                                   () => this.OnPropertyChanged(nameof(this.AllExport)), new ExportVm[] { }, false));
                }
            }
        }
        ...
        public NotifyTaskCompletion<ConcurrentObservableCollection<EmailVm>> Emails
        {
            get
            {
                lock (this._emailsLock)
                {
                    return this._emails
                           ?? (this._emails =
                               new NotifyTaskCompletion<ConcurrentObservableCollection<EmailVm>>(
                                   () => Task.Run(() => this.GetEmails()),
        ...
        public NotifyTaskCompletion<ConcurrentObservableCollection<CallVm>> Calls
        {
            get
            {
                lock (this._callsLock)
                {
                    return this._calls
                           ?? (this._calls =
                               new NotifyTaskCompletion<ConcurrentObservableCollection<CallVm>>(
                                   () => Task.Run(() => this.GetCalls()),
            ...
            get
            {
                lock (this._chatMessagesLock)
                {
                    return this._chatMessages
                           ?? (this._chatMessages =
                               new NotifyTaskCompletion<ConcurrentObservableCollection<ChatMessageVm>>(
                                   () => Task.Run(() => this.GetChatMessages()),
                                   () => this.OnPropertyChanged(nameof(this.ChatMessages))));
                }
            }
        }
        ...
        public NotifyTaskCompletion<string> Period
        {
            get
            {
                lock (this._periodLock)
                {
                    return this._period
                           ?? (this._period =
                               new NotifyTaskCompletion<string>(() => Task.Run(() => this.GetPeriod()),
        ...
        private string GetPeriod()
        {
            ...
            //
            // var min = results.FirstOrDefault(i => i.Export.TimeStampFirst != DateTime.MinValue)?.Export?.TimeStampFirst;
            // var max = results.LastOrDefault(i => i.Export.TimeStampLast != DateTime.MinValue)?.Export?.TimeStampLast;
            //
                ...
                lock (this._dataTypesDistributionLock)
                {
                    return this._dataTypesDistribution
                           ?? (this._dataTypesDistribution =
                               new NotifyTaskCompletion<KeyValue<string, long>[]>(
                                   () => Task.Run(() => this.GetDataTypesDistribution()),
                                   () => this.OnPropertyChanged(nameof(this.DataTypesDistribution)), false));
                }
            }
        }
        ...
        private KeyValue<string, long>[] GetDataTypesDistribution()
        {
            ...
            if (recursiveResults.Any())
            {
                ...
                var result = (from d in data
                    ...
                    into dg
                    select new KeyValue<string, long>(string.IsNullOrWhiteSpace(dg.Key) ? "(unknown)" : dg.Key,
                        dg.Sum(x => x.Value))).ToArray();
                return result;
            }
            ...
            get
            {
                lock (this._dataSummaryLock)
                {
                    return this._dataSummary
                           ?? (this._dataSummary =
                               new NotifyTaskCompletion<DataSummaryItem[]>(() => Task.Run(() => this.GetDataSummary()),
                                   () => this.OnPropertyChanged(nameof(this.DataSummary)), false));
                }
            }
        }
                ...
                lock (this._exportsDistributionLock)
                {
                    return this._exportsDistribution
                           ?? (this._exportsDistribution =
                               new NotifyTaskCompletion<KeyValue<DateTime, long>[]>(
                                   () => Task.Run(() => this.GetExportsDistribution()),
                                   () => this.OnPropertyChanged(nameof(this.ExportsDistribution)), false));
                }
            }
        }
        ...
        private KeyValue<DateTime, long>[] GetExportsDistribution()
        {
            return (from p in (ExportVm[]) this.AllExport
                group p by new DateTime(p.TimeStamp.Year, p.TimeStamp.Month, p.TimeStamp.Day, p.TimeStamp.Hour,
                    p.TimeStamp.Minute, p.TimeStamp.Second, DateTimeKind.Utc)
                into pg
                orderby pg.Key
        ...
        [IgnoreAutoChangeNotification]
        public RelayCommand<ExportVm> CNavigateToExportDetail => new RelayCommand<ExportVm>(exportVm =>
            this.NavigationService.Show(typeof(ExportDetailVm), exportVm));

        [IgnoreAutoChangeNotification]
        public RelayCommand<ExportVm> CSelectExport =>
            new RelayCommand<ExportVm>(exportVm => this.SelectedExport = exportVm);
        ...
        private void ExportGroups_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            this.Updated();
        }

        private void ExportGroupsOnCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            foreach (var exportGroupObj in e.NewItems)
            {
        ...
        private void ExportResults_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            this.Updated();
        }

        private void ExportsOnCollectionChanged(object sender,
