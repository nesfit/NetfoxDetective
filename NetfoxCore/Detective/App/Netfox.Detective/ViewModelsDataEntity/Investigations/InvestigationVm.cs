...
namespace Netfox.Detective.ViewModelsDataEntity.Investigations
{
    public class InvestigationVm : ConversationsVm<Investigation>, IInitializable, IInvestigationVm
    {
        ...
        public InvestigationVm(IWindsorContainer applicationWindsorContainer, ISnooperFactory snooperFactory,
            IFrameworkControllerFactory frameworkControllerFactory, Investigation model, ExportService exportService,
            IDetectiveMessenger messenger,
            ISerializationPersistor<Investigation> investigationSerializationPersistor) : base(
            applicationWindsorContainer, model, exportService)
        {
            ...
            this.SourceLogs =
                new ViewModelVirtualizingIoCObservableCollection<SourceLogVm, SourceLog>(this.Investigation.SourceLogs,
                    this.InvestigationWindsorContainer);

            this.OperationLogs =
                new ViewModelVirtualizingIoCObservableCollection<OperationLogVm, OperationLog>(
                    this.Investigation.OperationLogs, this.InvestigationWindsorContainer);
            this.Captures =
                new ViewModelVirtualizingIoCObservableCollection<CaptureVm, PmCaptureBase>(this.Investigation.Captures,
                    this.InvestigationWindsorContainer);

            this.ConversationsGroups =
                new ViewModelVirtualizingIoCObservableCollection<ConversationsGroupVm, ConversationsGroup>(
                    this.Investigation.ConversationsGroups,
                    this.InvestigationWindsorContainer);
            this.ConversationsGroups.CollectionChanged += this.ConversationsGroups_CollectionChanged;

            this.ExportGroups =
                new ViewModelVirtualizingIoCObservableCollection<ExportGroupVm, ExportGroup>(
                    this.Investigation.ExportsGroups, this.InvestigationWindsorContainer);
        ...
        public override ViewModelVirtualizingIoCObservableCollection<SnooperVm, ISnooper> AvailableSnoopers
        {
            get
            {
                if (this._availableSnoopers != null)
                {
                ...
                try
                {
                    this._availableSnoopers = new ViewModelVirtualizingIoCObservableCollection<SnooperVm, ISnooper>(
                        this.SnooperFactory.AvailableSnoopers,
                        this.InvestigationWindsorContainer);
                }
                catch (Exception ex)
                {
        ...
        public ViewModelVirtualizingIoCObservableCollection<OperationLogVm, OperationLog> OperationLogs
        {
            get;
            private set;
        }
        ...
        public CaptureVm CurrentCapture
        {
            ...
            set
            {
                if (this._currentCapture == value)
                {
                    return;
                }

                if (value != null && !this.Captures.Contains(value))
                {
                    return;
                }
        ...
        public ConcurrentObservableHashSet<CaptureVm> SelectedCaptureVms { get; } =
            new ConcurrentObservableHashSet<CaptureVm>();
        ...
        public ConversationsGroupVm CurrentConversationsGroupVm
        {
            ...
            set
            {
                if (this._currentConversationsGroupVm == value)
                {
                    return;
                }
                ...
                Task.Run(async () =>
                {
                    if (this._currentConversationsGroupVm == null)
                    {
                        return;
                    }
        ...
        public ConcurrentObservableHashSet<ConversationsGroupVm> SelectedConversationsGroupVms { get; } =
            new ConcurrentObservableHashSet<ConversationsGroupVm>();
        ...
        public ExportGroupVm CurrentExportGroup
        {
            ...
            set
            {
                ...
                Task.Run(async () =>
                {
                    if (this._currentExportGroup == null)
                    {
                        return;
                    }
        ...
        public ConcurrentObservableHashSet<ExportGroupVm> SelectedExportGroupVms { get; } =
            new ConcurrentObservableHashSet<ExportGroupVm>();

        public long ToExportTotalSize => 0;

        public ConcurrentObservableCollection<ILxConversation> ToExportConversations =>
            new ConcurrentObservableCollection<ILxConversation>();
        ...
        public ICommand CRemoveCapture =>
            new RelayCommandAsync(this.RemoveCaptureExecuteAsync, this.CanRemoveCaptureExecute);

        public ICommand CProtocolsRecognition =>
            new RelayCommand(this.CProtocolsRecognitionxecute, this.CanCProtocolsRecognitionExecute);
        ...
        public async Task Initialize()
        {
            ...
            {
                this._messenger.Register<AddConversationToExportMessage>(this,
                    this.AddConversationToExportMessageReceived);
        ...
        {
            foreach (var pcap in this.Investigation.InvestigationInfo.SourceCaptureDirectoryInfo.GetDirectories()
        ...
        {
            foreach (var log in this.Investigation.InvestigationInfo.SourceLogsDirectoryInfo.GetDirectories()
                .SelectMany(d => d.EnumerateFiles()))
            {
        ...
        public async Task AddCaptureAsync(string filePath)
        {
            ...
            var targetDirectoryInfo =
                this.Investigation.InvestigationInfo.SourceCaptureDirectoryInfo.CreateSubdirectory(fileName + "-" +
                    Guid.NewGuid());
            targetDirectoryInfo.Create();
            var coppiedFileInfo =
                new FileInfo(PathHelper.CombineLongPath(targetDirectoryInfo.FullName, captureFilePathInfo.Name));
        ...
        {
            if (conversationsGroup == null)
            {
                return;
            }

            Task.Factory.StartNew(() =>
                this.ToExportConversations.AddItems(conversationsGroup.ConversationsGroup.Conversations));
        }

        public void AddConversationToExport(ILxConversation conversation) =>
            this.ToExportConversations.Add(conversation);
        ...
        public async Task AddLogFile(string filePath)
        {
            ...
            var targetDirectoryInfo =
                this.Investigation.InvestigationInfo.SourceLogsDirectoryInfo.CreateSubdirectory(fileName + "-" +
                    Guid.NewGuid());
            targetDirectoryInfo.Create();
        ...
        public async Task AddSourceLog()
        {
            ...
            var logFiles =
                this.SystemServices.OpenFilesDialog("", ".wdat", "Warcraft log (*.wdat)|*.wdat;|All Files (*.*)|*.*");
            foreach (var logFile in logFiles)
            {
                await this.AddLogFile(logFile);
            }
        }
            ...
            await Task.Run(() =>
            {
                Parallel.ForEach(this.Captures, async captureVm =>
                {
                    try
                    {
                        ...
                        {
                            this.Logger?.Error(
                                $"Capture checksum is not correct {Path.GetFileName(captureVm.Capture.FilePath)}");
                        }
                    }
                    catch (Exception ex)
                    {
                        this.Logger?.Error($"Unable to check capture {captureVm.Capture.FileInfo.FullName} checksum!",
                            ex);
                    }
                });
            });
        ...
        public async Task RemoveCaptureAsync(CaptureVm capture)
        {
            await this.RemoveCaptureAsync(capture?.EncapsulatedModel as PmCaptureBase);
        }
        ...
        public void RemoveConversationsGroup(ConversationsGroupVm conversationsGroup)
        {
            if (conversationsGroup != null)
            {
                this.Investigation.ConversationsGroups.Remove(conversationsGroup.ConversationsGroup);
            }
        }
        ...
        protected override async Task ActualizeExportSet(List<object> parameters)
        {
            ...
            await this.ExportService.Export(this.Investigation.SourceLogs, this.CurrentInvestigationSelectedExporters(),
                null, true, showExportedObjectsDuringExportation);
        }
        ...
        private bool CanCProtocolsRecognitionExecute()
        {
            return true;
        }
        ...
        {
            return this.CurrentExportGroup != null;
        }
        ...
        private void AddConversationToExportMessageReceived(AddConversationToExportMessage msg)
        {
            Task.Factory.StartNew(() =>
            {
                ...
                if (cvm != null)
                {
                    this.ToExportConversations.Add(cvm.Conversation);
                }
            });
            this.NavigationService.Show(typeof(SelectiveExportVm));
        }
        ...
        private void ConversationsGroups_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            {
                foreach (var cgVm in this.ConversationsGroups)
                {
                    cgVm.Captures = this.Captures;
                }
        ...
        private void CProtocolsRecognitionxecute()
        {
            this.ProtocolsRecognition();
        }
        ...
        private async Task RemoveCaptureExecuteAsync()
        {
            ...
            {
                await this.RemoveCaptureAsync(this.CurrentCapture);
            }
        }

        private void RemoveGroup()
        {
            if (this.CurrentExportGroup != null)
            {
                this.RemoveExportGroup(this.CurrentExportGroup);
            }
        }
        ...
        private async Task UpdateInvestigationAnalyzers()
        {
            await Task.Run(() =>
            {
                ...
                {
                    foreach (var availableAnalyzerType in this.CrossContainerHierarchyResolver.AvailableAnalyzerTypes
                        .Where(t => typeof(IAnalyzerInvestigation).IsAssignableFrom(t)))
                    {
                        try
                        {
                            if (this.ApplicationShell.AvailableAnalyzers.Any(a => a.GetType() == availableAnalyzerType))
                                return;
                            this.ApplicationShell.AvailableAnalyzers.Add(this.InvestigationWindsorContainer.Resolve(
                                availableAnalyzerType, new Dictionary<string, object>
                                {
