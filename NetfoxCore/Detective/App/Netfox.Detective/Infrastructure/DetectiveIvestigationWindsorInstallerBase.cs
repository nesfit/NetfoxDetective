...
{
    public abstract class DetectiveIvestigationWindsorInstallerBase : IWindsorInstaller,
        IDetectiveIvestigationWindsorInstaller
    {
        ...
        {
            container.Register(this.FromAssemblyDescriptor.BasedOn<IAnalyzerInvestigation>()
                .WithServiceFromInterface(typeof(IAnalyzerInvestigation)).WithServiceSelf().LifestyleSingleton());
        }

        protected internal virtual void SnooperRegister(IWindsorContainer container)
        {
            container.Register(this.FromAssemblyDescriptor.BasedOn<ISnooper>()
                .WithServiceFromInterface(typeof(ISnooper)).WithServiceSelf().LifestyleTransient());
        }

        protected internal virtual void ServicesRegister(IWindsorContainer container)
        {
            container.Register(this.FromAssemblyDescriptor.BasedOn<IStartableInvestigationService>()
                .WithServiceDefaultInterfaces().WithServiceSelf().LifestyleSingleton()
                .Configure(component => component.Start()));
            container.Register(this.FromAssemblyDescriptor.BasedOn<IInvestigationService>()
                .WithServiceDefaultInterfaces().WithServiceSelf().LifestyleSingleton());
        }
        ...
        {
            container.Register(this.FromAssemblyDescriptor.BasedOn<DetectiveInvestigationPaneViewModelBase>()
                .If(IsContainingMultiparameterConstructor).WithServiceDefaultInterfaces()
        ...
        {
            container.Register(this.FromAssemblyDescriptor.BasedOn<DetectiveDataEntityViewModelBase>()
                .If(IsContainingMultiparameterConstructor).WithServiceDefaultInterfaces()
                .LifestyleCustom<PerEntityLifestyleManager>());
            container.Register(this.FromAssemblyDescriptor.BasedOn<DetectiveDataEntityViewModelBase>()
        ...
        private static bool IsContainingMultiparameterConstructor(Type type)
        {
            return type.GetConstructors().Any(c => c.GetParameters().Count() > 1);
        }

        protected internal virtual void DetectiveIvestigationDataEntityPaneViewModelBaseRegister(
            IWindsorContainer container)
        {
            container.Register(this.FromAssemblyDescriptor.BasedOn<DetectiveIvestigationDataEntityPaneViewModelBase>()
                .If(IsContainingMultiparameterConstructor).WithServiceDefaultInterfaces()
                .LifestyleCustom<PerEntityLifestyleManager>());
            container.Register(this.FromAssemblyDescriptor.BasedOn<DetectiveIvestigationDataEntityPaneViewModelBase>()
                .WithServiceDefaultInterfaces().LifestyleTransient());
        ...
        protected static FromAssemblyDescriptor GetFromAssemblyDescriptorForAssemblisDeclaringTypes(
            IEnumerable<Type> types)
        {
            return Classes.FromAssemblyInDirectory(
                new AssemblyFilter(".", "*Snooper*").FilterByAssembly(assembly =>
                    assembly.ExportedTypes.Intersect(types).Any()));
        }
    }
}