...
namespace Netfox.Detective.ViewModels
{
    ...
    {
        private readonly ConcurrentDictionary<TKey, TimeLimitedCacheValue> _cache =
            new ConcurrentDictionary<TKey, TimeLimitedCacheValue>();
        ...
        public TimeLimitedCachee(TimeSpan expiryTimeSpan)
        {
            this._ExpiryTimeSpan = expiryTimeSpan;
        }
        ...
        public TValue GetValueOrSetAsync(TKey key, Func<TValue> func, Action<Task> continueWith, TValue defaultValue,
            object lockToken, TimeSpan? funcTimespanLimit = null)
        {
            TimeLimitedCacheValue timeLimitedValue;
            if (this._cache.TryGetValue(key, out timeLimitedValue))
            {
            ...
            lock (lockToken)
            {
                ...
                try
                {
                    this._cache.AddOrUpdate(key, k => timeLimitedValue = new TimeLimitedCacheValue(defaultValue),
                        (k, v) => timeLimitedValue = v);
                    ...
                    else
                    {
                        ...
                        var waitingTask = Task.Factory
                            .StartNew(() => { waitedTask.Wait((TimeSpan) funcTimespanLimit); })
                            .ContinueWith(continueWith);
                    }
        ...
        private void CreateTimer()
        {
            if (Interlocked.CompareExchange(ref this._ExpiryRunning, 1, 0) == 0)
            {
                ...
                {
                    this._ExpiryTimer = new Timer(o => this.Expire(), null, this._ExpiryTimeSpan, this._ExpiryTimeSpan);
                }
            }
        }
        ...
        {
            if (Interlocked.CompareExchange(ref this._ExpiryRunning, 0, 1) != 1)
            {
                return;
            }
        ...
        private void Expire()
        {
            if (Interlocked.CompareExchange(ref this._ExpiryInProgress, 1, 0) == 1)
            {
                return;
            }
            ...
            try
            {
                ...
                var toRemove = this._cache.Where(pair =>
                    pair.Value.LastAccess != DateTime.MinValue && pair.Value.LastAccess < maxAge).ToArray();
