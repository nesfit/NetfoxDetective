...
namespace Netfox.Detective.ViewModels.Frame
{
    ...
    public class FrameVm : DetectiveDataEntityViewModelBase
    {
        ...
        public FrameVm(WindsorContainer applicationWindsorContainer, PmFrameBase model) : base(
        ...
        public int FrameFlowSize
        {
            get
            {
                if (this.FwFrame == null)
                {
                    return 0;
                }

                if (this.Direction == FrameDirection.Unknow || this.Direction == FrameDirection.Up)
                {
                ...
                while (p != null)
                {
                    ...
                    if (p is TcpPacket)
                    {
                        ...
                        yield return new GenericFiledVm
                        {
                            ...
                            Value =
                                $"sp: {q.SourcePort}, dp: {q.DestinationPort}, sn: {q.SequenceNumber}, ack: {q.AcknowledgmentNumber}",
                            Content = new object[]
                            {
                                new
                                {
        ...
        private static string RawAsciiBytes(byte[] bytes, int index, int count, bool allowLineBreak = false)
        {
            ...
            {
                if (bytes[i] > 0x20 && bytes[i] < 0x7E || (allowLineBreak && (bytes[i] == 0x0D || bytes[i] == 0x0A)))
                {
                    result += (char) bytes[i];
                }
                else
