...
namespace Netfox.Web.Tests
{
    ...
    public class UserManagementTests : ViewModelTestBase
    {
        ...
        {
            this.Configuration.RouteTable.Add("Settings_UserManagement", "settings/User_management", "user.dothtml",
                null);
            ...
            var viewModel = this.Container.Resolve<UserViewModel>(new Dictionary<string, object>()
            {
                {"Context", context},
                {"Password", "testpass"},
            });
            ...
            catch (DotvvmInterruptRequestExecutionException ex)
            {
                ...
                using (var uow = this.UnitOfWorkProvider.Create())
                {
                    ...
                    var passHash = LoginFacade.SHA256Hash("testpass");
                    Assert.AreEqual(
                        ctx.Users.Count(u =>
                            u.Surname == "test" && u.Firstname == "test" && u.Username == "testuser" &&
                            u.Password == passHash), 1);
                }
        ...
        public void TestRemoveUser()
        {
            this.UserFacade.Save(
                new UserDTO()
                {
                    Firstname = "test", IsEnable = true, Username = "testuser", Surname = "test",
            ...
            var viewModel = this.Container.Resolve<UserManagementViewModel>(new Dictionary<string, object>()
            {
                {"Context", context},
            });
            viewModel.Init();
        ...
        public void TestRemoveSelectedUsers()
        {
            for (var i = 1; i <= 5; i++)
            {
                this.UserFacade.Save(
                    new UserDTO()
                    {
                        Firstname = "test" + i, IsEnable = true, Username = "testuser" + i, Surname = "test" + i,
                        RoleId = this.AdminRoleID
                    }, "testpass");
            }
        ...
        {
            this.Configuration.RouteTable.Add("Settings_UserManagement", "settings/User_management", "article.dothtml",
                null);
            var context = new TestDotvvmRequestContext()
            {
                Configuration = this.Configuration,
            ...
            catch (DotvvmInterruptRequestExecutionException ex)
            {
                ...
                using (var uow = this.UnitOfWorkProvider.Create())
                {
                    ...
                    Assert.AreEqual(
                        ctx.Users.Count(u =>
                            u.Surname == "test2" && u.Firstname == "test" && u.Username == "testuser" && !u.IsEnable &&
                            u.Password == passHash), 1);
                }
        ...
        public void TestChangePassword()
        {
            ...
            var viewModel = this.Container.Resolve<ProfileViewModel>(new Dictionary<string, object>()
            {
                {"Context", context},
                {"Username", "testuser"}
            });
            ...
            {
                var ctx = EntityFrameworkUnitOfWork.TryGetDbContext<NetfoxWebDbContext>(this.UnitOfWorkProvider);
                var passHash = LoginFacade.SHA256Hash("newtestpass");
                Assert.AreEqual(
                    ctx.Users.Count(u =>
                        u.Surname == "test" && u.Firstname == "test" && u.Username == "testuser" &&
                        u.Password == passHash), 1);
        ...
        public void TestLoginEmptyInput()
        {
            ...
            var viewModel = this.Container.Resolve<LoginViewModel>(new Dictionary<string, object>()
            {
                {"Context", context},
        ...
        public void TestNotActiveLogin()
        {
            ...
            this.UserFacade.Save(
                new UserDTO()
                {
                    Firstname = "test", IsEnable = false, Username = "testuser", Surname = "test",
                    RoleId = this.AdminRoleID
                }, "testpass");
