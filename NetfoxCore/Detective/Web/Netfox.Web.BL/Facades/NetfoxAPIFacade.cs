...
namespace Netfox.Web.BL.Facades
{
    public class NetfoxAPIFacade
    {
        ...
        public NetfoxAPIFacade(INetfoxWebSettings settings, IWindsorContainer container, Guid investigationId, string appPath)
        {
            _settings = settings;
            ...
            this.InvestigationInfo.InvestigationsDirectoryInfo = new DirectoryInfo(appPath + _settings.InvestigationsFolder);
            this.InvestigationInfo.SqlConnectionStringBuilder = new SqlConnectionStringBuilder(_settings.ConnectionString)
            {
                InitialCatalog = settings.InvestigationPrefix + investigationId
            };
            ...
            this.L7Conversations = this.WindsorContainer.Resolve<VirtualizingObservableDBSetPagedCollection<L7Conversation>>(new Dictionary<string, object>()
            {
                {
        ...
        {
            var snoopers = availableSnoopers.Except(this.Stats.GetExportedProtocols(captureId));
            var exportedProtocols = this.SnooperFactory.AvailableSnoopers.Where(s => snoopers.Contains(s.GetType().FullName)).ToArray();

            var L7 = this.L7Conversations.Where(c => c.Captures.Any(cc => cc.Id == captureId)).ToArray();
            this.FrameworkController.ExportData(exportedProtocols, L7, new DirectoryInfo(appPath + _settings.InvestigationsFolder + _settings.InvestigationPrefix + InvestigationInfo.Guid + "/Exports"), true);

            var snooperExports = this.SnooperExports.Where(e => e.SourceCaptureId == captureId);
            this.FrameworkController.ExportData(exportedProtocols, snooperExports, new DirectoryInfo(appPath + _settings.InvestigationsFolder + _settings.InvestigationPrefix + InvestigationInfo.Guid + "/Exports"));
        ...
        {
            var dir = new DirectoryInfo(@appPath + _settings.InvestigationsFolder);
            dir.Create();
            return dir;
        }
