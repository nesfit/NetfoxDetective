...
{
    public class L4ConversationQuery : NetfoxQueryBase<L4ConversationDTO>
    {
        ...
        public L4ConversationQuery(NetfoxUnitOfWorkProvider unitOfWorkProvider) : base(unitOfWorkProvider)
        {
        }

        private IQueryable<L4Conversation> GetBaseQuery()
        {
            var query = (IQueryable<L4Conversation>) this.unitOfWorkProvider.TryGetContext().Set<L4Conversation>();
        ...
        protected override IQueryable<L4ConversationDTO> GetQueryable()
        {
            // Base Query
            var query = this.GetBaseQuery();

            return query.ProjectTo<L4ConversationDTO>();
        }

        public void FillDataSet(GridViewDataSet<L4ConversationDTO> dataSet)
        {
            ...
            dataSet.Items = this.GetBaseQuery().OrderBy(i => i.FirstSeen)
                ...
                .Take(dataSet.PagingOptions.PageSize).ToList().AsQueryable().ProjectTo<L4ConversationDTO>().ToList();
        }

        private IQueryable<L4Conversation> ApplyFilters(IQueryable<L4Conversation> baseQuery)
        {
            var query = baseQuery;
            if (this.Filters != null)
            {
                if (this.Filters.FramesTo != this.Filters.FrameMax && this.Filters.FramesTo != 0)
                {
                    query = query.Where(c => c.Frames.Count <= this.Filters.FramesTo);
                }

                if (this.Filters.FramesFrom != this.Filters.FrameMin && this.Filters.FramesTo != 0)
                {
                    query = query.Where(c => c.Frames.Count >= this.Filters.FramesFrom);
                }

                if (this.Filters.BytesTo != this.Filters.BytesMax && this.Filters.BytesTo != 0)
                {
                    query = query.Where(c => c.ConversationFlowStatistics.Sum(cs => cs.Bytes) <= this.Filters.BytesTo);
                }

                if (this.Filters.BytesFrom != this.Filters.BytesMin && this.Filters.BytesFrom != 0)
                {
                    query = query.Where(c =>
                        c.ConversationFlowStatistics.Sum(cs => cs.Bytes) >= this.Filters.BytesFrom);
                }

                if (!String.IsNullOrEmpty(this.Filters.DurationFrom))
                {
                    var durFrom = DateTime.ParseExact(this.Filters.DurationFrom, "dd.MM.yyyy HH:mm:ss",
                        new DateTimeFormatInfo());
                    if (DateTime.Compare(durFrom, this.Filters.DurationMin) != 0)
                    {
                        query = query.Where(c => c.FirstSeen >= durFrom);
                    }
                }

                if (!String.IsNullOrEmpty(this.Filters.DurationTo))
                {
                    var durTo = DateTime.ParseExact(this.Filters.DurationTo, "dd.MM.yyyy HH:mm:ss",
                        new DateTimeFormatInfo());
                    if (DateTime.Compare(durTo, this.Filters.DurationMax) != 0)
                    {
                        query = query.Where(c => c.LastSeen <= durTo);
                    }
                }
                ...
                if (!String.IsNullOrEmpty(this.Filters.SearchText))
                {
                    ...
                    if (count == 0)
                    {
                        ...
                        if (this.Filters.SearchText.Count(f => f == '.') > 0)
                        {
                            ...
                            if (success)
                            {
                                ...
                                query = query.Where(c =>
                                    c.SourceEndPointIPAddressData == addressBytes ||
                                    c.DestinationEndPointIPAddressData == addressBytes);
                            }
                        }
                        else // port
                        {
                            ...
                            {
                                query = query.Where(c =>
                                    c.SourceEndPointPort == port || c.DestinationEndPointPort == port);
                            }
                        }
                    }
                    else if (count == 1) //IPEndpoint
                    {
                        ...
                        query = query.Where(c =>
                            (c.SourceEndPointIPAddressData == searchTextBytes && c.SourceEndPointPort == port) ||
                            (c.DestinationEndPointIPAddressData == searchTextBytes &&
                             c.DestinationEndPointPort == port));
                    }
                    else //IPv6Endpoint
                    {
                        ...
                        if (splitEndpoint.Count() > 1)
                        {
                            ...
                            var port = Int64.Parse(splitEndpoint[1]);
                            query = query.Where(c =>
                                (c.SourceEndPointIPAddressData == addressBytes && c.SourceEndPointPort == port) ||
                                (c.DestinationEndPointIPAddressData == addressBytes &&
                                 c.DestinationEndPointPort == port));
                        }
                        else
        ...
        private IQueryable<L4Conversation> AddSorting(IQueryable<L4Conversation> baseQuery)
        {
            var query = baseQuery;

            // TODO: 200210425 - repair sorting
