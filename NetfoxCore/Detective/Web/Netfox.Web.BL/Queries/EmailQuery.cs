...
namespace Netfox.Web.BL.Queries
{
    public class EmailQuery : NetfoxQueryBase<ExportEmailDTO>
    {
        ...
        public void FillDataSet(GridViewDataSet<ExportEmailDTO> dataSet)
        {
            ...
            dataSet.Items = this.GetBaseQuery().OrderBy(i => i.Id)
                .Skip(dataSet.PagingOptions.PageIndex * dataSet.PagingOptions.PageSize).Take(dataSet.PagingOptions.PageSize)
                .ToList().AsQueryable().ProjectTo<ExportEmailDTO>().ToList();
        }
        ...
        public void FillAttachmentsOfEmail(GridViewDataSet<EmailAttachmentDTO> dataSet, Guid emailId)
        {
            ...
            dataSet.Items = attachments.OrderBy(i => i.FirstSeen)
