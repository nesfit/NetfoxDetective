...
namespace EntityFramework.InMemory
{
    public class TestDbSet : DbSet, IQueryable, IDbAsyncEnumerable
    {
        ...
        {
            this._data.Remove(GetIModel(entity).Id);
            return entity;
        }
...
#pragma warning restore 672
    ...
    public class TestDbSet<TEntity> : DbSet<TEntity>, IQueryable, IEnumerable<TEntity>, IDbAsyncEnumerable<TEntity>,
        IList, INotifyCollectionChanged where TEntity : class
    {
        ...
        public TestDbSet(Action<TEntity> addCallback, Action<IEnumerable<TEntity>> addBulkCallback,
            Action<TEntity> removeCallback) : this(addCallback, addBulkCallback, removeCallback, null)
        {
        }

        public TestDbSet(Action<TEntity> addCallback, Action<IEnumerable<TEntity>> addBulkCallback,
            Action<TEntity> removeCallback, TestDbSet dbSet = null)
        {
            if (dbSet == null)
            {
                this.Data = new InterfaceListWrapper<TEntity>(new ConcurrentObservableCollection<TEntity>());
                this._query = this.Data.AsQueryable().Where(p =>
                    (p.GetType().IsSubclassOf(typeof(TEntity))) || (p.GetType() == typeof(TEntity)));
            }
            else
            {
        ...
        public IEnumerable<TEntity> AddRange(IEnumerable items)
        {
            return this.AddRange(items, true);
        }
        ...
        {
            return ((IList) this.Data).Contains(value);
        }
        ...
        {
            return ((IList) this.Data).IndexOf(value);
        }

        void IList.Insert(int index, object value)
        {
            ((IList) this.Data).Insert(index, value);
        }
