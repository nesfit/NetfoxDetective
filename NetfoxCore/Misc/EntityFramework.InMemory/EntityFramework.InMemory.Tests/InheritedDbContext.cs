...
{
    public sealed class InheritedDbContext : BloggingContext
    {
        ...
        public InheritedDbContext(DbConnection existingConnection, bool contextOwnsConnection) : base(existingConnection, contextOwnsConnection) { }
        ...
        public InheritedDbContext(ObjectContext objectContext, bool dbContextOwnsObjectContext) : base(objectContext, dbContextOwnsObjectContext) { }
        ...
        {
            var mockDbSetType = typeof(TestDbSet<>).MakeGenericType(dbSetPropertyInfo.PropertyType.GenericTypeArguments.First());
        ...
        {
            var dbSetPropertyInfo = this.GetAllDbSetProperties().FirstOrDefault(p => p.PropertyType.GenericTypeArguments.Contains(typeof(TEntity)));
            if(dbSetPropertyInfo == null) { return null; }
            var dbSet = dbSetPropertyInfo.GetValue(this) as TestDbSet<TEntity>;
            if(dbSet == null) { dbSet = this.CreateDbSet<TEntity>(dbSetPropertyInfo); }
            return dbSet;
        }
        ...
        public override DbSet Set(Type entityType)
        {
            ...
            if(ret == null) throw new NotSupportedException("Unit test conversion of DbSet<TEntity> to DbSet is not supported... se MSDN for details");
            return ret;
            ;
        }
        #endregion
