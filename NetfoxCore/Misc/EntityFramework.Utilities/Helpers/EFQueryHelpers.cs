...
namespace EntityFramework.Utilities.Helpers
{
    public static class EFQueryHelpers
    {
        ...
        /// <returns></returns>
        public static EFUQueryable<T> IncludeEFU<T, TChild>(this IQueryable<T> query, DbContext context,
            ...
            where TChild : class
        {
            ...
            var cSpaceType =
                cSpaceTables.Single(t =>
                    t.Name == typeof(T)
                        .Name); //Use single to avoid any problems with multiple tables using the same type
            ...
            var childProp =
                SetCollectionModifiersAndGetChildProperty<T, TChild>(collectionSelector, childCollectionModifiers);
        ...
        private static IQueryable<TChild> ApplyChildCollectionModifiers<TChild>(IQueryable<TChild> childQ,
            List<MethodCallExpression> childCollectionModifiers) where TChild : class
        {
            foreach (var item in childCollectionModifiers)
            {
                switch (item.Method.Name)
                {
                    ...
                    case "ThenByDescending":
                        ...
                        break;
                    default:
                        throw new NotSupportedException("The method " + item.Method.Name +
                                                        " is not supported in the child query");
                }
            }
        ...
        private static PropertyInfo SetCollectionModifiersAndGetChildProperty<T, TChild>(
            Expression<Func<T, IEnumerable<TChild>>> collectionSelector,
            List<MethodCallExpression> childCollectionModifiers)
            where T : class
            where TChild : class
        {
        ...
        private static Func<TChild, object> GetForeignKeyGetter<T, TChild>(
            System.Collections.ObjectModel.ReadOnlyCollection<EntityType> cSpaceTables)
            where T : class
            where TChild : class
        {
        ...
        private static PropertyInfo GetFKProperty<T, TChild>(
            System.Collections.ObjectModel.ReadOnlyCollection<EntityType> cSpaceTables)
            where T : class
            where TChild : class
        {
            var cSpaceChildType =
                cSpaceTables.Single(t =>
                    t.Name == typeof(TChild)
                        .Name); //Use single to avoid any problems with multiple tables using the same type
            var fk = cSpaceChildType.NavigationProperties
                .First(n => n.ToEndMember.GetEntityType().Name == typeof(T).Name).GetDependentProperties().First();
            var fkInfo = typeof(TChild).GetProperty(fk.Name);
            return fkInfo;
        }

        private static IQueryable<TChild> SortQuery<TChild>(IQueryable<TChild> query, MethodCallExpression item,
            string method)
        {
        ...
        private static Expression<Func<T, IEnumerable<TChild>>> GetRootEntityToChildCollectionSelector<T, TChild>(
            EntityType cSpaceType)
            where T : class
            where TChild : class
        {
            var parameter = Expression.Parameter(typeof(T), "t");
            var memberExpression = Expression.Property(parameter,
                cSpaceType.NavigationProperties.First(p => p.ToEndMember.GetEntityType().Name == typeof(TChild).Name)
                    .Name);
