...
using IQueryProvider = EntityFramework.Utilities.Interfaces.IQueryProvider;
...
namespace EntityFramework.Utilities
{
    public class SqlQueryProvider : IQueryProvider
    {
        ...
        public string GetUpdateQuery(QueryInformation predicateQueryInfo, QueryInformation modificationQueryInfo)
        {
            ...
            {
                updateSql = string.Join(" = ",
                    update.Split(new string[] {" = "}, StringSplitOptions.RemoveEmptyEntries).Reverse());
            }


            return string.Format("UPDATE [{0}].[{1}] SET {2} {3}", predicateQueryInfo.Schema, predicateQueryInfo.Table,
                updateSql, predicateQueryInfo.WhereSql);
        }
        ...
        public void UpdateItems<T>(IEnumerable<T> items, string schema, string tableName,
            IList<ColumnMapping> properties, DbConnection storeConnection, int? batchSize,
            UpdateSpecification<T> updateSpecification)
        {
            ...
            var str = string.Format("CREATE TABLE {0}.[{1}]({2}, PRIMARY KEY ({3}))", schema, tempTableName,
                string.Join(", ", columns), pkConstraint);
            ...
            var pks = properties.Where(p => p.IsPrimaryKey)
