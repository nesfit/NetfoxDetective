...
namespace EntityFramework.BulkInsert.Helpers
{
        ...
        <T> : IDataReader
    {
        ...
        public MappedDataReader(IEnumerable<T> enumerable, IEfBulkInsertProvider provider)
        {
            ...
            {
                throw new Exception($"No table mappings provided. Type {nameof(T)}.");
            }
            ...
            {
                throw new Exception($"All mappings must have same table name. Type {nameof(T)}.");
            }
            ...
            foreach (var kvp in tableMappings)
            {
                ...
                foreach (var col in propertyMaps)
                {
                    ...
                    else
                    {
                        ...
                        propertyExpression =
                            propNames.Skip(1).Aggregate(propertyExpression, Expression.PropertyOrField);
        ...
        public object GetValue(int i)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (this._currentEntityTypeSelectors.Count > i)
                        value = this._currentEntityTypeSelectors[i](this._enumerator.Current);
                    else return null;
