...
namespace Netfox.Persistence
{
    ...
    protected BaseContex(IWindsorContainer windsorContainer, SqlConnectionStringBuilder sqlConnectionStringBuilder) :
        base(sqlConnectionStringBuilder.ConnectionString)
    {
            ...
            try
            {
                ...
                foreach (var type in entityTypes)
                {
                    try
                    {
                        ...
                        this.WindsorContainer.Register(Component.For(dbSetType).LifestyleSingleton().Instance(dbSet)
                            .OnlyNewServices());
                    }
                    catch (Exception ex)
                    {
    ...
    public virtual async Task BulkInsertBuffered<TEntity>(IEnumerable<TEntity> entities, BulkInsertOptions options)
        where TEntity : class
    {
    ...
    public virtual async Task BulkInsertBuffered<T>(IEnumerable<T> entities, SqlBulkCopyOptions sqlBulkCopyOptions,
        int? batchSize = null) where T : class
    {
    ...
    public virtual async Task BulkInsertBuffered<T>(IEnumerable<T> entities, IDbTransaction transaction,
        SqlBulkCopyOptions sqlBulkCopyOptions = SqlBulkCopyOptions.Default, int? batchSize = null) where T : class
    {
        var options = new BulkInsertOptions {SqlBulkCopyOptions = sqlBulkCopyOptions};
        if (batchSize.HasValue)
        {
    ...
    public virtual void BulkInsert<TEntity>(IEnumerable<TEntity> entities, BulkInsertOptions options)
        where TEntity : class
    {
    ...
    public virtual void BulkInsert<TEntity>(IEnumerable<TEntity> entities, SqlBulkCopyOptions sqlBulkCopyOptions,
        int? batchSize = null) where TEntity : class
    {
        var options = new BulkInsertOptions {SqlBulkCopyOptions = sqlBulkCopyOptions};
        if (batchSize.HasValue)
        {
    ...
    public virtual void BulkInsert<TEntity>(IEnumerable<TEntity> entities, IDbTransaction transaction,
        SqlBulkCopyOptions sqlBulkCopyOptions = SqlBulkCopyOptions.Default, int? batchSize = null) where TEntity : class
    {
        var options = new BulkInsertOptions {SqlBulkCopyOptions = sqlBulkCopyOptions};
        if (batchSize.HasValue)
        {
    ...
    public virtual async Task BulkInsertAsync<TEntity>(IEnumerable<TEntity> entities, BulkInsertOptions options)
        where TEntity : class
    {
    ...
    public virtual Task BulkInsertAsync<T>(IEnumerable<T> entities, SqlBulkCopyOptions sqlBulkCopyOptions,
        int? batchSize = null) where T : class
    {
        var options = new BulkInsertOptions {SqlBulkCopyOptions = sqlBulkCopyOptions};
        if (batchSize.HasValue)
    ...
    public virtual Task BulkInsertAsync<T>(IEnumerable<T> entities, IDbTransaction transaction,
        SqlBulkCopyOptions sqlBulkCopyOptions = SqlBulkCopyOptions.Default, int? batchSize = null) where T : class
    {
        var options = new BulkInsertOptions {SqlBulkCopyOptions = sqlBulkCopyOptions};
        if (batchSize.HasValue)
    ...
    protected override void OnModelCreating(DbModelBuilder model)
    {
        ...
        model.Properties<DateTime>().Configure(c => c.HasColumnType("datetime2"));

        this.OnModelCreatingImportPersistenceTypes(model);
    }
    ...
    private void OnModelCreatingImportPersistenceTypes(DbModelBuilder modelBuilder)
    {
        ...
        try
        {
            ...
            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies()
                .Concat(snooperLoader.GetSnoopersAssemblies()))
            {
                try
                {
                    var entityTypes = assembly.GetTypes()
                        .Where(t => t.GetCustomAttributes(typeof(PersistentAttribute), true).Any());

                    foreach (var type in entityTypes)
                    {
                        try
                        {
                            var entityMethodReturn = entityMethod.MakeGenericMethod(type)
            ...
            modelBuilder.Types()
                .Where(t => typeof(SnooperExportBase).IsAssignableFrom(t) && t != typeof(SnooperExportBase))
                .Configure(t => t.ToTable(t.ClrType.Name));

            //Table per Type mapping for SnooperExportedObjectBase
            modelBuilder.Types()
                .Where(t => typeof(SnooperExportedObjectBase).IsAssignableFrom(t) &&
                            t != typeof(SnooperExportedObjectBase)).Configure(t => t.ToTable(t.ClrType.Name));
        }
        catch (Exception ex)
        {
            Debug.WriteLine(ex);
    ...
    protected virtual void OnDbSetChanged(DbSetChangedArgs args)
    {
        this.DbSetChanged?.Invoke(this, args);
    }
