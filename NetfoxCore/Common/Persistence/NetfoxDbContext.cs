...
namespace Netfox.Persistence
{
    ...
    {
        public NetfoxDbContext(IWindsorContainer windsorContainer,
            SqlConnectionStringBuilder sqlConnectionStringBuilder) : base(windsorContainer, sqlConnectionStringBuilder)
        {
            this.Configuration.LazyLoadingEnabled = true;
            this.InitializeIWindsorContainerChanger();

            this.L3ConversationStatistics = this.Set<L3ConversationStatistics>()
                .Where(s => !(s is L4ConversationStatistics) && !(s is L7ConversationStatistics));
            this.L4ConversationStatistics =
                this.Set<L4ConversationStatistics>().Where(s => !(s is L7ConversationStatistics));
        ...
        private void InitializeIWindsorContainerChanger()
        {
            ...
            {
                if (e?.Entity is IWindsorContainerChanger entity)
                {
                    entity.InvestigationWindsorContainer = this.WindsorContainer;
                }
            };
        }
        ...
        protected override void OnModelCreating(DbModelBuilder model)
        {
            ...
            foreach (var t in WindsorContainer.ResolveAll<ISnooper>().SelectMany(i => i.GetType().Assembly.GetTypes()).Where(i => i.GetCustomAttribute<PersistentAttribute>() != null))
            {
                model.RegisterEntityType(t);
            }
            
            #region Mandatory configuration for all bulkinserted types that needs junction table!!!

            model.Entity<PmCaptureBase>().HasMany(p => p.L3Conversations).WithMany(l => l.Captures).Map(pl =>
            {
            ...
            model.Entity<PmCaptureBase>().HasMany(p => p.L4Conversations).WithMany(l => l.Captures).Map(pl =>
            {
                pl.MapLeftKey(nameof(PmCaptureConversation.PmCaptureId));
                pl.MapRightKey(nameof(PmCaptureConversation.ConversationId));
            ...
            model.Entity<PmCaptureBase>().HasMany(p => p.L7Conversations).WithMany(l => l.Captures).Map(pl =>
            {
                pl.MapLeftKey(nameof(PmCaptureConversation.PmCaptureId));
                pl.MapRightKey(nameof(PmCaptureConversation.ConversationId));
        ...
        protected ConcurrentDictionary<Type, AutoRefreshArgs> AutoRefreshArgsDictionary { get; } =
            new ConcurrentDictionary<Type, AutoRefreshArgs>();
        ...
        public void AutoRefresh(Type dbSetType)
        {
            ...
            try
            {
                ...
                var autoRefreshArgs = this.AutoRefreshArgsDictionary.GetOrAdd(dbSetType,
                    type => new AutoRefreshArgs(dbSetType, this.GetSqlCommandForAutoRefrest(dbSetType)));
                ...
                var sqlConnection =
                    new SqlConnection(this.Database.Connection
                        .ConnectionString); //cannot be in using because it would close channel for backwards notification
