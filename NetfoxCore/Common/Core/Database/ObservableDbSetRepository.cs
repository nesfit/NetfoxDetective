...
{
    public class ObservableDbSetRepository<T> : IRepository<T>, INotifyImmediately, INotifyCollectionChanged,
        ISynchronized where T : class, IEntity
    {
        ...
        public bool IsNotifyImmidiately
        {
            ...
            set
            {
                if (value)
                {
                    ...
                    {
                        iNotifyCollectionChangedDbSet.CollectionChanged -=
                            this
                                .INotifyCollectionChangedDbSetOnCollectionChanged; //To ensure that only one instance is subscribed
                        iNotifyCollectionChangedDbSet.CollectionChanged +=
                            this.INotifyCollectionChangedDbSetOnCollectionChanged;
                ...
                else
                {
                    if (this.DbSet is INotifyCollectionChanged iNotifyCollectionChangedDbSet)
                    {
                        iNotifyCollectionChangedDbSet.CollectionChanged -=
                            this.INotifyCollectionChangedDbSetOnCollectionChanged;
                        this.DbContext.DbSetChanged -=
                            this.DbContextOnDbSetChanged; //To ensure that only one instance is subscribed
                        this.DbContext.DbSetChanged += this.DbContextOnDbSetChanged;
                    }
                }
        ...
        private void INotifyCollectionChangedDbSetOnCollectionChanged(object o,
        ...
        private void DbContextOnDbSetChanged(DbContext sender, DbSetChangedArgs args)
        {
            this.OnChange();
        }
        ...
        public async Task<int> CountAsync() =>
            await this.ConcurrentLock(async () => await this.QueryWithoutEagerLoading.CountAsync());
                    ...
                    AppDomain.CurrentDomain.GetAssemblies()?
                        .SelectMany(assembly =>
                            assembly.GetTypes().Where(t =>
                                t.GetCustomAttributes(typeof(PersistentAttribute), true).Any()))
                        .ToArray());
        ...
        private IQueryable<T> QueryWithoutEagerLoading =>
            this.QueryFilter == null ? this.DbSet : this.DbSet.Where(this.QueryFilter);
        ...
        private IQueryable<T> Query
        {
            get
            {
                ...
                {
                    query = query.Where(this.QueryFilter);
                }

                if (this.EagerLoadProperties != null)
                {
                    query = this.EagerLoadProperties.Aggregate(query,
                        (current, propertyName) => current.Include(propertyName));
                }
        ...
        {
            return this.ConcurrentLock(() => this.Query.Any(i => i.Id == item.Id));
        }

        public async Task<bool> ContainsAsync(T item)
        {
            return await this.ConcurrentLock(async () => await this.Query.AnyAsync(i => i.Id == item.Id));
        }
        ...
        public int IndexOf(T item)
        {
            ...
            return this.ConcurrentLock(() => this.Query.AsEnumerable()
                .TakeWhile(i => i.Id != item.Id).Count());
        }

        public async Task<int> IndexOfAsync(T item)
        {
            //TODO fix to async
            return await this.ConcurrentLock(async () =>
                await Task.Run(() => this.Query.AsEnumerable().TakeWhile(i => i.Id != item.Id).Count()));
        }
        ...
        public T[] SkipTake(int skip, int take)
        {
            return this.ConcurrentLock(() => this.Query.Skip(skip).Take(take).ToArray());
        }

        public async Task<T[]> SkipTakeAsync(int skip, int take)
        {
            return await this.ConcurrentLock(async () => await this.Query.Skip(skip).Take(take).ToArrayAsync());
        }
