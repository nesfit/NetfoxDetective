...
namespace Netfox.FrameworkAPI.Tests
{
    ...
    public abstract class UnitTestBaseSetupFixture
    {
        ...
        private const string TEST_PATH = @"C:\NetfoxTemp";
        private const int MAX_TEST_AGE_DAYS = 2;
        ...
        public FileInfo PrepareCaptureForProcessing(string filePath)
        {
            ...
            var newFile =
                originalFile.CopyTo(Path.Combine(
                    this.InvestigationInfo.SourceCaptureDirectoryInfo
                        .CreateSubdirectory(originalFile.Name + Guid.NewGuid()).FullName,
                    originalFile.Name));
            return newFile;
        }
        ...
        public virtual void TearDown()
        {
            ...
            Console.WriteLine("Timeduration: " + duration.Hours + ":" + duration.Minutes + ":" + duration.Seconds +
        ...
        /// <summary>
        /// For retention purposes. Maximal test result age is defined by MAX_TEST_AGE_DAYS const.
        /// </summary>
        [OneTimeTearDown]
        public virtual void OneTimeTearDown()
        {
            ...
            foreach (var dir in dirs)
            {
                ...
                if(di.LastAccessTime.AddDays(MAX_TEST_AGE_DAYS) < DateTime.Now)
        ...
        private void SetUp()
        {
            ...
            this.WindsorContainer =
                new WindsorContainer("BaseUnitTestWC", new DefaultKernel(), new DefaultComponentInstaller());
            this.WindsorContainer.Register(Component
                .For<NetfoxJsonSettings, INetfoxSettings, ILoggerSettings, IAppSettings>().LifestyleSingleton());
            this.WindsorContainer.Register(Component.For<IWindsorContainer, WindsorContainer>()
                .Instance(this.WindsorContainer));
            //this.WindsorContainer.Install(new NetfoxFrameworkApiWindsorInstaller());
            this.WindsorContainer.Install(FromAssembly.InDirectory(new AssemblyFilter("."),
        ...
        {
            var dir = new DirectoryInfo(TEST_PATH);
            dir.Create();
            return dir;
        }

        protected void SingleTestWaitOne()
        {
            SingleTestMutex.WaitOne();
        }
    }
}