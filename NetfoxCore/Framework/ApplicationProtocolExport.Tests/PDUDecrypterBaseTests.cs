...
namespace ApplicationProtocolExport.Tests
{
    ...
    public class PDUDecrypterBaseTests : SnooperBaseTests
    {
        ...
        public void TestDecrypterCbc_first_message()
        {
            ...
            var path = PcapPath.GetPcap(PcapPath.Pcaps.webmail_webmail_gmail_pcapng);
            //var captureFile = this.PrepareCaptureForProcessing(Pcaps.Default.webmail_webmail_gmail_pcapng);
            var captureFile = this.PrepareCaptureForProcessing(path);
        ...
        public void TestDecrypterCbc_second_message()
        {
            /* TLS 1.2 CBC conversation */
            var captureFile =
                this.PrepareCaptureForProcessing(PcapPath.GetPcap(PcapPath.Pcaps.webmail_webmail_gmail_pcapng));
            this.FrameworkController.ProcessCapture(captureFile);
            var conversations = this.L7Conversations.ToArray();
            var decryptersCbcTls12 = new List<PDUDecrypterBase>();
            this.AddCypherKeyToConversations(conversations, decryptersCbcTls12);
        ...
        public void TestDecrypterGcm()
        {
            ...
            var captureFile =
                this.PrepareCaptureForProcessing(PcapPath.GetPcap(PcapPath.Pcaps.webmail_webmail_live_pcapng));
            this.FrameworkController.ProcessCapture(captureFile);
            var conversations = this.L7Conversations.ToArray();
            var decryptersGcmTls12 = new List<PDUDecrypterBase>();
            this.AddCypherKeyToConversations(conversations, decryptersGcmTls12);

            foreach (var d in decryptersGcmTls12)
        ...
        public void TestDecrypterRc4_first_message()
        {
            ...
            var captureFile =
                this.PrepareCaptureForProcessing(PcapPath.GetPcap(PcapPath.Pcaps.webmail_webmail_yahoo_rc4_pcapng));
            this.FrameworkController.ProcessCapture(captureFile);
            var conversations = this.L7Conversations.ToArray();
        ...
        private void AddCypherKeyToConversations(L7Conversation[] conversations,
            List<PDUDecrypterBase> decryptersStreamTls12)
        {
            this.SetPrivateKeyToConversations(conversations, PcapPath.GetKey(PcapPath.KeyPath.fw_pk_pem));
