...
namespace ApplicationProtocolExport.Tests
{
    ...
    public class PDUStreamBaseTests : FrameworkBaseTests
    {
        ...
        {
            base.SetUpInMemory();
            var captureFile = this.PrepareCaptureForProcessing(PcapPath.GetPcap(PcapPath.Pcaps.pcap_mix_icq1_cap));
        ...
        {
            var captureFile = this.PrepareCaptureForProcessing(PcapPath.GetPcap(PcapPath.Pcaps.sip_caps_sip_rtcp_small_pcap));
            this.FrameworkController.ProcessCapture(captureFile);
            this._conversations = this.L7Conversations.Where(c => c?.ApplicationTags.Contains("SIP") ?? false).ToArray();
            var counter = 0;
            string line;
        ...
        {
            var captureFile = this.PrepareCaptureForProcessing(PcapPath.GetPcap(PcapPath.Pcaps.sip_caps_sip_rtcp_pcap));
            this.FrameworkController.ProcessCapture(captureFile);
            this._conversations = this.L7Conversations.Where(c => c?.ApplicationTags.Contains("SIP") ?? false).ToArray();
            var counter = 0;
            string line;
