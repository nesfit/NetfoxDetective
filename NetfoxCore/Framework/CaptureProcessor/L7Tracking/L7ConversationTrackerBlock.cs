...
namespace Netfox.Framework.CaptureProcessor.L7Tracking
{
        ...
        ISourceBlock<L7PDU>
    {
        ...
        private BufferBlock<L7ConversationStatistics> L7ConversationStatistics { get; } =
            new BufferBlock<L7ConversationStatistics>();

        private BufferBlock<L7PDU> L7Pdus { get; } = new BufferBlock<L7PDU>();

        private BufferBlock<PmCaptureL7Conversation> PmCaptureL7Conversations { get; } =
            new BufferBlock<PmCaptureL7Conversation>();

        private ActionBlock<PmFrameBase> ProcessFrame { get; }
        private ActionBlock<L4ConversationExtended> ProcessL4Conversation { get; }

        private ConcurrentObservableHashSet<L4ConversationExtended> L4ConversationsForFinishL7Tracking { get; } =
            new ConcurrentObservableHashSet<L4ConversationExtended>();
        ...
        /// </summary>
        /// <inheritdoc/>
        public L4Conversation ConsumeMessage(DataflowMessageHeader messageHeader, ITargetBlock<L4Conversation> target,
            out bool messageConsumed)
        {
            return ((ISourceBlock<L4Conversation>) this.L4Conversations).ConsumeMessage(messageHeader, target,
                out messageConsumed);
        }
        ...
        /// </summary>
        /// <inheritdoc/>
        public L7Conversation ConsumeMessage(DataflowMessageHeader messageHeader, ITargetBlock<L7Conversation> target,
            out bool messageConsumed)
        {
            return ((ISourceBlock<L7Conversation>) this.L7Conversations).ConsumeMessage(messageHeader, target,
                out messageConsumed);
        }
        ...
        public L7ConversationStatistics ConsumeMessage(
            ...
            out bool messageConsumed)
        {
            return ((ISourceBlock<L7ConversationStatistics>) this.L7ConversationStatistics).ConsumeMessage(
        ...
        {
            return ((ISourceBlock<L7ConversationStatistics>) this.L7ConversationStatistics).ReserveMessage(
        ...
        public void ReleaseReservation(DataflowMessageHeader messageHeader,
            ITargetBlock<L7ConversationStatistics> target)
        {
            ((ISourceBlock<L7ConversationStatistics>) this.L7ConversationStatistics).ReleaseReservation(messageHeader,
                target);
        }
        ...
        /// </summary>
        /// <inheritdoc/>
        public L7PDU ConsumeMessage(DataflowMessageHeader messageHeader, ITargetBlock<L7PDU> target,
            out bool messageConsumed)
        {
        ...
        public PmCaptureL7Conversation ConsumeMessage(
            ...
            out bool messageConsumed)
        {
            return ((ISourceBlock<PmCaptureL7Conversation>) this.PmCaptureL7Conversations).ConsumeMessage(messageHeader,
        ...
        {
            return ((ISourceBlock<PmCaptureL7Conversation>) this.PmCaptureL7Conversations).ReserveMessage(messageHeader,
                target);
        }
        ...
        public void ReleaseReservation(DataflowMessageHeader messageHeader,
            ITargetBlock<PmCaptureL7Conversation> target)
        {
            ((ISourceBlock<PmCaptureL7Conversation>) this.PmCaptureL7Conversations).ReleaseReservation(messageHeader,
                target);
        }
        ...
        private async Task<bool> PostL7ConversationsIfAny(IEnumerable<L7Conversation> l7Conversations)
        {
            ...
            foreach (var l7Conversation in l7Conversations)
            {
                ...
                foreach (var l7ConversationCapture in l7Conversation.Captures)
                {
                    ...
                    await this.PmCaptureL7Conversations.SendAsync(
                        new PmCaptureL7Conversation(l7ConversationCapture.Id, l7Conversation.Id));
                }
                ...
                foreach (var l7PDU in l7Conversation.L7PDUs)
                {
                    await this.L7Pdus.SendAsync(l7PDU);
                }

                foreach (var l7ConversationFrame in l7Conversation.Frames)
                {
                    await this.Frames.SendAsync(l7ConversationFrame);
                }
            }
