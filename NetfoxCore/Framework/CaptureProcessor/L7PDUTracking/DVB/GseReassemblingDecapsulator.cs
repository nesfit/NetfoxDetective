...
namespace Netfox.Framework.CaptureProcessor.L7PDUTracking.DVB
{
    ...
    internal class GseReassemblingDecapsulator
    {
        ...
        /// </summary>
        private readonly Dictionary<short, Dictionary<byte, GseReassemblyBuffer>> _reassemblyBuffers =
            new Dictionary<short, Dictionary<byte, GseReassemblyBuffer>>();
        ...
        public IEnumerable<PmFrameBase> Process([Required] BaseBandFrame baseBandFrame,
            [Required] ICollection<PmFrameBase> encapsulatingFrames)
        {
            ...
            foreach (var gsePacket in baseBandFrame.UserPackets)
            {
                ...
                switch (gsePacket.Type)
                {
                    ...
                    case PacketType.Start:
                        ...
                        {
                            this._reassemblyBuffers[si]
                                .Add(gsePacket.Header.FragmentID.Value, new GseReassemblyBuffer());
                        }
                        ...
                        this._reassemblyBuffers[si][gsePacket.Header.FragmentID.Value]
                            .Add(gsePacket, encapsulatingFrames);
                        break;
                    ...
                    case PacketType.End:
                        ...
                        if (this._reassemblyBuffers[si].ContainsKey(gsePacket.Header.FragmentID.Value))
                        {
                            ...
                            var decapsulated = this._reassemblyBuffers[si][gsePacket.Header.FragmentID.Value]
                                .Retrieve();
        ...
        private class GseReassemblyBuffer
        {
            ...
            {
                if (!gsePacket.IsGseFragment)
                {
                    throw new ArgumentException("GSE packet is not a GSE fragment packet.", nameof(gsePacket));
                }
                ...
                switch (gsePacket.Type)
                {
                    ...
                    case PacketType.Complete:
                        throw new ArgumentException(
                            $"GSE packet with type {gsePacket.Type} cannot be added to GseReassemblyBuffer.",
                            nameof(gsePacket));
                    case PacketType.Start:
                        if (this._buffer.Any())
                        {
                            throw new ArgumentException(
                                $"GSE packet with type {gsePacket.Type} cannot be added to not empty GseReassemblyBuffer.",
                                nameof(gsePacket));
                        }
                    ...
                    case PacketType.End:
                        if (this._buffer.Any())
                        {
                            if (!(this._buffer.Last().Type == PacketType.Start ||
                                  this._buffer.Last().Type == PacketType.Intermediate))
                            {
                                throw new ArgumentException(
                                    $"GSE packet with type {gsePacket.Type} cannot be added to GseReassemblyBuffer after packet with type {this._buffer.Last().Type}.",
                                    nameof(gsePacket));
                            }
                        }
                        else
                        {
                            throw new ArgumentException(
                                $"GSE packet with type {gsePacket.Type} cannot be added to empty GseReassemblyBuffer.",
                                nameof(gsePacket));
                        }
                ...
                {
                    if (!this._encapsulatingFrames.Contains(ef))
                    {
                        this._encapsulatingFrames.Add(ef);
                    }
                }
            }
            ...
            public void BaseBandProcessed()
            {
                if (this._baseBandsSinceFirstFragment < 255)
                {
                    if (this._buffer.Any())
                    {
                        this._baseBandsSinceFirstFragment++;
                    }
                }
                else
                {
            ...
            public bool IsCompleted
            {
                get
                {
                    ...
                    return this._buffer.Any()
                           && this._buffer.First().Type == PacketType.Start &&
                           this._buffer.Last().Type == PacketType.End
                           && (this._buffer.Count == 2 || this._buffer.GetRange(1, this._buffer.Count - 2)
                               .TrueForAll(a => a.Type == PacketType.Intermediate));
                }
            }
        }
    }
