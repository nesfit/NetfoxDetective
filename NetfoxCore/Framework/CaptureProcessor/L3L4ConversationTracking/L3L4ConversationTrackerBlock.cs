...
namespace Netfox.Framework.CaptureProcessor.L3L4ConversationTracking
{
            ...
            ISourceBlock<L4ConversationStatistics>
    {
        ...
        private readonly ConcurrentDictionary<L3ConversationExtended, L3ConversationExtended> _l3FlowsTable =
            new ConcurrentDictionary<L3ConversationExtended, L3ConversationExtended>();

        private readonly ConcurrentDictionary<L4FlowKey, L4ConversationExtended> _l4ConversationTable =
            new ConcurrentDictionary<L4FlowKey, L4ConversationExtended>();

        public L3L4ConversationTrackerBlock(IL3ConversationFactory l3ConversationFactory,
            IL4ConversationFactory l4ConversationFactory, NBARProtocolPortDatabase nbarProtocolPortDatabase)
        {
            ...
            this.ProcessPacketActionBlock.Completion.ContinueWith(t =>
            {
                ...
                else
                {
                    if (this.NullL3Conversation.Frames.Any())
                    {
                        ...
                        this.L3ConversationStatistics.SendAsync(this.NullL3Conversation.DownConversationStatistic)
                            .Wait();
                    }
        ...
        private BufferBlock<PmCaptureL3Conversation> PmCaptureL3Conversations { get; } =
            new BufferBlock<PmCaptureL3Conversation>();

        private ISourceBlock<PmCaptureL3Conversation> _PmCaptureL3Conversations => this.PmCaptureL3Conversations;

        private BufferBlock<L3ConversationStatistics> L3ConversationStatistics { get; } =
            new BufferBlock<L3ConversationStatistics>();

        private ISourceBlock<L3ConversationStatistics> _L3ConversationStatistics => this.L3ConversationStatistics;

        private BufferBlock<L4ConversationExtended> L4Conversations { get; } =
            new BufferBlock<L4ConversationExtended>();

        private ISourceBlock<L4ConversationExtended> _L4Conversations => this.L4Conversations;

        private BufferBlock<PmCaptureL4Conversation> PmCaptureL4Conversations { get; } =
            new BufferBlock<PmCaptureL4Conversation>();

        private ISourceBlock<PmCaptureL4Conversation> _PmCaptureL4Conversations => this.PmCaptureL4Conversations;

        private BufferBlock<L4ConversationStatistics> L4ConversationStatistics { get; } =
            new BufferBlock<L4ConversationStatistics>();
        ...
        private L4ConversationExtended CreateL4Conversation(PmFrameBase baseConversationFrame,
            L3Conversation l3Conversation, long l4FlowMTU)
        {
            ...
            return this.L4ConversationFactory.Create(baseConversationFrame.IpProtocol, l3Conversation, sourceEndPoint,
                destinationEndPoint, l4FlowMTU);
        }
        ...
        private L3ConversationExtended L3Provider(PmFrameBase packet)
        {
            switch (packet.IpProtocol)
            {
                ...
                default:
                    return this.L3ConversationFactory.Create(PmFrameBase.NullEndPoint.Address,
                        PmFrameBase.NullEndPoint.Address);
            }
        }
        ...
        {
            L4FlowKey l4FlowKey;
            if (packet.IpProtocol == IPProtocolType.IP || packet.IpProtocol == IPProtocolType.IPV6 ||
                packet.IpProtocol == IPProtocolType.TCP
                || packet.IpProtocol == IPProtocolType.UDP || packet.IpProtocol == IPProtocolType.ICMP ||
                packet.IpProtocol == IPProtocolType.ICMPV6
        ...
        {
            if (
                !(packet.IpProtocol == IPProtocolType.TCP || packet.IpProtocol == IPProtocolType.UDP ||
                  packet.IpProtocol == IPProtocolType.IP
                  || packet.IpProtocol == IPProtocolType.IPV6 || packet.IpProtocol == IPProtocolType.ICMP ||
                  packet.IpProtocol == IPProtocolType.ICMPV6
        ...
        private async Task L3Updater(L3ConversationExtended l3Conversation, PmFrameBase frame)
        {
            ...
            if (l3Conversation.CapturesHashSet.Add(frame.PmCapture))
            {
                ...
                await this.PmCaptureL3Conversations.SendAsync(new PmCaptureL3Conversation(frame.PmCapture.Id,
                    l3Conversation.Id));
            }
        }
        ...
        private async Task L4Updater(L4ConversationExtended l4Conversation, PmFrameBase frame)
        {
            try
            {
                ...
                if (l4Conversation.CapturesHashSet.Add(frame.PmCapture))
                {
                    ...
                    await this.PmCaptureL4Conversations.SendAsync(
                        new PmCaptureL4Conversation(frame.PmCapture.Id, l4Conversation.Id));
                }
            }
            catch (Exception e)
            {
        ...
        public DataflowMessageStatus OfferMessage(DataflowMessageHeader messageHeader, PmFrameBase messageValue,
            ISourceBlock<PmFrameBase> source, bool consumeToAccept)
        {
        ...
        {
            this.ProcessPacketActionBlock.Complete();
        }

        public void Fault(Exception exception)
        {
            ((IDataflowBlock) this.ProcessPacketActionBlock).Fault(exception);
        }
        ...
        public IDisposable LinkTo(ITargetBlock<L3Conversation> target, DataflowLinkOptions linkOptions)
        {
            return this._L3Conversations.LinkTo(target, linkOptions);
        }

        public L3Conversation ConsumeMessage(DataflowMessageHeader messageHeader, ITargetBlock<L3Conversation> target,
            out bool messageConsumed)
        {
            return this._L3Conversations.ConsumeMessage(messageHeader, target, out messageConsumed);
        }

        public bool ReserveMessage(DataflowMessageHeader messageHeader, ITargetBlock<L3Conversation> target)
        {
            return this._L3Conversations.ReserveMessage(messageHeader, target);
        }
        ...
        public IDisposable LinkTo(ITargetBlock<L4ConversationExtended> target, DataflowLinkOptions linkOptions)
        {
            return this._L4Conversations.LinkTo(target, linkOptions);
        }

        public L4ConversationExtended ConsumeMessage(DataflowMessageHeader messageHeader,
            ITargetBlock<L4ConversationExtended> target, out bool messageConsumed)
        {
        ...
        {
            return this._ProcessedFrames.LinkTo(target, linkOptions);
        }

        public PmFrameBase ConsumeMessage(DataflowMessageHeader messageHeader, ITargetBlock<PmFrameBase> target,
            out bool messageConsumed)
        {
        ...
        {
            return this._ProcessedFrames.ReserveMessage(messageHeader, target);
        }

        public void ReleaseReservation(DataflowMessageHeader messageHeader, ITargetBlock<PmFrameBase> target)
        {
            this._ProcessedFrames.ReleaseReservation(messageHeader, target);
        }
        ...
        public IDisposable LinkTo(ITargetBlock<L3ConversationStatistics> target, DataflowLinkOptions linkOptions)
        {
            return this._L3ConversationStatistics.LinkTo(target, linkOptions);
        }

        public L3ConversationStatistics ConsumeMessage(DataflowMessageHeader messageHeader,
            ITargetBlock<L3ConversationStatistics> target, out bool messageConsumed)
        {
            return this._L3ConversationStatistics.ConsumeMessage(messageHeader, target, out messageConsumed);
        }

        public bool ReserveMessage(DataflowMessageHeader messageHeader, ITargetBlock<L3ConversationStatistics> target)
        {
            return this._L3ConversationStatistics.ReserveMessage(messageHeader, target);
        }

        public void ReleaseReservation(DataflowMessageHeader messageHeader,
            ITargetBlock<L3ConversationStatistics> target)
        {
            this._L3ConversationStatistics.ReleaseReservation(messageHeader, target);
        }
        ...
        public IDisposable LinkTo(ITargetBlock<L4ConversationStatistics> target, DataflowLinkOptions linkOptions)
        {
            return this._L4ConversationStatistics.LinkTo(target, linkOptions);
        }

        public L4ConversationStatistics ConsumeMessage(DataflowMessageHeader messageHeader,
            ITargetBlock<L4ConversationStatistics> target, out bool messageConsumed)
        {
            return this._L4ConversationStatistics.ConsumeMessage(messageHeader, target, out messageConsumed);
        }

        public bool ReserveMessage(DataflowMessageHeader messageHeader, ITargetBlock<L4ConversationStatistics> target)
        {
            return this._L4ConversationStatistics.ReserveMessage(messageHeader, target);
        }

        public void ReleaseReservation(DataflowMessageHeader messageHeader,
            ITargetBlock<L4ConversationStatistics> target)
        {
            this._L4ConversationStatistics.ReleaseReservation(messageHeader, target);
        }
        ...
        public IDisposable LinkTo(ITargetBlock<PmCaptureL3Conversation> target, DataflowLinkOptions linkOptions)
        {
            return this._PmCaptureL3Conversations.LinkTo(target, linkOptions);
        }

        public PmCaptureL3Conversation ConsumeMessage(DataflowMessageHeader messageHeader,
            ITargetBlock<PmCaptureL3Conversation> target, out bool messageConsumed)
        {
            return this._PmCaptureL3Conversations.ConsumeMessage(messageHeader, target, out messageConsumed);
        }

        public bool ReserveMessage(DataflowMessageHeader messageHeader, ITargetBlock<PmCaptureL3Conversation> target)
        {
            return this._PmCaptureL3Conversations.ReserveMessage(messageHeader, target);
        }

        public void ReleaseReservation(DataflowMessageHeader messageHeader,
            ITargetBlock<PmCaptureL3Conversation> target)
        {
            this._PmCaptureL3Conversations.ReleaseReservation(messageHeader, target);
        }
        ...
        public IDisposable LinkTo(ITargetBlock<PmCaptureL4Conversation> target, DataflowLinkOptions linkOptions)
        {
            return this._PmCaptureL4Conversations.LinkTo(target, linkOptions);
        }

        public PmCaptureL4Conversation ConsumeMessage(DataflowMessageHeader messageHeader,
            ITargetBlock<PmCaptureL4Conversation> target, out bool messageConsumed)
        {
            return this._PmCaptureL4Conversations.ConsumeMessage(messageHeader, target, out messageConsumed);
        }

        public bool ReserveMessage(DataflowMessageHeader messageHeader, ITargetBlock<PmCaptureL4Conversation> target)
        {
            return this._PmCaptureL4Conversations.ReserveMessage(messageHeader, target);
        }

        public void ReleaseReservation(DataflowMessageHeader messageHeader,
            ITargetBlock<PmCaptureL4Conversation> target)
        {
            this._PmCaptureL4Conversations.ReleaseReservation(messageHeader, target);
        }
