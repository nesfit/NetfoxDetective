...
namespace Netfox.Framework.CaptureProcessor.Captures
{
    public abstract class PmCaptureProcessorBlockBase : IPmCaptureProcessorBlockBase, ISourceBlock<PmFrameBase>
    {
        ...
        {
            await Task.WhenAll(Task.Run(async () => await this.ParseAndReateMetaFrames()),
                Task.Run(() => this.ComputeHash()));
        ...
        {
            this.PmCapture.PcapHashOriginal = this.PmCapture.ComputePcapHash();
        }
        ...
        public async Task CreateAndAddToMetaFramesVirtualFrame(PmFrameBase pmFrame, PmPacket parentPacket)
        {
            ...
            switch (pmFrame.IpProtocol)
            {
                case IPProtocolType.GRE:
                    ...
                    virtualFrame = new PmFrameVirtual(pmFrame.PmCapture, pmFrame.FrameIndex, pmFrame.TimeStamp,
                        pmFrame.IncludedLength, (pmFrame.L4Offset + gre.Header.Length));
                    break;

                case IPProtocolType.IPV6:
                    virtualFrame = new PmFrameVirtual(pmFrame.PmCapture, pmFrame.FrameIndex, pmFrame.TimeStamp,
                        pmFrame.IncludedLength, (pmFrame.L4Offset));
                    break;
        ...
        {
            this._pmMetaFramesBufferBlock.Fault(exception);
        }
