...
namespace Netfox.Framework.CaptureProcessor.Captures
{
    ...
    /// </remarks>
    [Serializable]
    internal class PmCaptureProcessorBlockPcap : PmCaptureProcessorBlockBase
    {
        ...
        public override DateTime? GetFirstTimeStamp()
            => ConvertUnixTimeToDate(ConvertTcpdToUnixTime(
                (Int32) (this.GetTcpdPckTimestampSeconds(24) + this._tcpdTimeZoneOffset),
                (Int32) this.GetTcpdPckTimestampUseconds(24)));
        ...
        private static TimeSpan ConvertDateToUnixTime(DateTime current) =>
            new TimeSpan(current.Ticks - new DateTime(1970, 1, 1).Ticks);
        ...
        private static TimeSpan ConvertTcpdToUnixTime(Int32 seconds, Int32 microseconds) =>
            new TimeSpan(0, 0, 0, seconds).Add(new TimeSpan(microseconds * 10));
        ...
        private void GetTcpdPckPacketHeaderRecord(PmFrameBase fr, out Int64 tsSec, out Int64 tsUsec, out Int64 inclLen,
            out Int64 origLen, out Byte[] frameData)
        {
        ...
        internal void PrintTcpdPacketHeaderRecord(PmFrameBase fr)
        {
            ...
            "Converted time> {0}".PrintInfoEol(
                ConvertUnixTimeToDate(ConvertTcpdToUnixTime((Int32) tsSec, (Int32) tsUsec))
...
*/
        ...
        private async Task ParseTcpdFrameTable()
        {
            ...
            do
            {
                ...
                await this.PmMetaFramesBufferBlock.SendAsync(new PmFramePcap(this.PmCapture, index, last,
                    this._pmLinkType,
                    ConvertUnixTimeToDate(ConvertTcpdToUnixTime(
                        (int) (this.GetTcpdPckTimestampSeconds(last) + this._tcpdTimeZoneOffset),
