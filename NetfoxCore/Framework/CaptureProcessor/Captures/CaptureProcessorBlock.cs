...
namespace Netfox.Framework.CaptureProcessor.Captures
{
    ...
    /// </remarks>
    internal class CaptureProcessorBlock : ISourceBlock<PmFrameBase>, ITargetBlock<FileInfo>,
        ISourceBlock<PmCaptureBase>
    {
        ...
        private PmCaptureFileType DetermineType(BinaryReader fileReader)
        {
            if (fileReader == null)
            {
                throw new ArgumentException("Parameter cannot be null", nameof(fileReader));
            }
        ...
        private PmCaptureProcessorBlockBase GetPmCaptureProcessor(FileInfo captureFileInfo)
        {
            ...
            using (var fileBinaryReader = new BinaryReader(Stream.Synchronized(new FileStream(captureFileInfo.FullName,
                FileMode.Open, FileAccess.Read, FileShare.Read))))
            {
                ...
                switch (fileType)
                {
                    ...
                    default:
                        throw new ControllerCaptureProcessorBase.UnknownFileType(
                            "CaptureProcessor file format can not be determined for " + captureFileInfo.FullName);
                }
        ...
        public DataflowMessageStatus OfferMessage(DataflowMessageHeader messageHeader, FileInfo messageValue,
            ISourceBlock<FileInfo> source, bool consumeToAccept)
        {
            return this._ProcessCaptureFileActionBlock.OfferMessage(messageHeader, messageValue, source,
                consumeToAccept);
        }

        public void Complete()
        {
            this._ProcessCaptureFileActionBlock.Complete();
        }

        public void Fault(Exception exception)
        {
            this._ProcessCaptureFileActionBlock.Fault(exception);
        }
        ...
        {
            return this._pmParsedFramesBufferBlock.LinkTo(target, linkOptions);
        }
        ...
        public PmCaptureBase ConsumeMessage(DataflowMessageHeader messageHeader, ITargetBlock<PmCaptureBase> target,
            out bool messageConsumed)
        {
