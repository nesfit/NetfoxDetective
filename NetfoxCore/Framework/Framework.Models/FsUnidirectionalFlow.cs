...
namespace Netfox.Framework.Models
{
    ...
    [NotMapped]
    [DataContract]
    public class FsUnidirectionalFlow
    {
        ...
        public IEnumerable<PmFrameBase> RealFrames
        {
            get
            {
                return
                    this.Frames.Where(f =>
                            f.PmFrameType != PmFrameType.VirtualBlank && f.PmFrameType != PmFrameType.VirutalNoise)
        ...
        public DateTime FirstSeen
        {
            get
            {
                return this.Frames.Any()
                    ? this.Frames.Min(frame => frame.TimeStamp)
                    : this.NonDataFrames.Min(frame => frame.TimeStamp);
            }
        }
        ...
        public DateTime LastSeen
        {
            ...
            {
                return this.Frames.Any()
                    ? this.Frames.Max(frame => frame.TimeStamp)
                    : this.NonDataFrames.Max(frame => frame.TimeStamp);
            }
        }
        ...
        public List<PmFrameBase> VirtualFrames =>
            this._virtualFrames ?? (this._virtualFrames = new List<PmFrameBase>());

        /// <summary> Gets the non data frames.</summary>
        /// <value> The non data frames.</value>
        public List<PmFrameBase> NonDataFrames =>
            this._nonDataFrames ?? (this._nonDataFrames = new List<PmFrameBase>());
