...
namespace Netfox.Framework.Models.Snoopers
{
    ...
    {
        private List<SnooperExportedObjectBase>
            _currentObjectBase; //TODO What does this mean? How could be more current export objects?
        ...
        public virtual List<SnooperExportedObjectBase> ExportObjects { get; set; } =
            new List<SnooperExportedObjectBase>();
        ...
        public SnooperExportedObjectBase CurrentObjectBase
        {
            ...
            internal set
            {
                if (this._currentObjectBase.Contains(value))
                {
                    throw new ExportedObjectAlreadyAdded();
                }
        ...
        public DateTime TimeStampFirst
        {
            get
            {
                ...
                if (this.ExportObjects != null && this.ExportObjects.Any())
                {
                    return this.ExportObjects.First().TimeStamp;
                }
        ...
        public DateTime TimeStampLast
        {
            get
            {
                if (this._timeStampFirst != DateTime.MinValue && this._timeStampFirst != DateTime.MaxValue)
                    return this._timeStampFirst;
                if (this.ExportObjects != null && this.ExportObjects.Any())
                {
                    return this.ExportObjects.Last().TimeStamp;
                }
        ...
        public void AddExportReport(
            ...
            this.AddExportReport(level, sourceComponent, description, null, sourceExportData, exception);

        public void CheckExportState()
        {
            if (this.ExportContext != ExportContext.Done)
            {
                throw new WrongExportContext();
            }
        ...
        public void OnAfterDataExporting()
        {
            ...
            if (this._currentObjectBase.Any())
            {
                foreach (var objectBase in this._currentObjectBase)
                {
                    Console.WriteLine(objectBase.ToString());
                }
        ...
        {
            // TODO: 20210429 - Uncomment if test variable debug evaluations timed out 
        ...
        public Guid SourceCaptureId
        {
            get
            {
                ...
                {
                    this._sourceCaptureId = (this.ExportSource is L7Conversation)
                        ? ((this.ExportSource as L7Conversation).Captures.First().Id)
                        : Guid.Empty;
                }

                if (this._sourceCaptureId == Guid.Empty)
                {
                    this._sourceCaptureId = (this.ExportSource is L7PDU)
                        ? ((this.ExportSource as L7PDU).L7Conversation.Captures.First().Id)
                        : Guid.Empty;
                }
