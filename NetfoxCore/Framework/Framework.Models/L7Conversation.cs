...
namespace Netfox.Framework.Models
{
    ...
    public class L7Conversation : ILxConversation, IWindsorContainerChanger
    {
        ...
        protected L7Conversation()
        {
        } //EF

        public L7Conversation(FsUnidirectionalFlow flow, DaRFlowDirection flowDirection,
            IWindsorContainer localWindsorContainer)
        {
        ...
        public L7Conversation(FsUnidirectionalFlow upFlow, FsUnidirectionalFlow downFlow,
            IWindsorContainer localWindsorContainer)
        {
            this.InvestigationWindsorContainer = localWindsorContainer;
        ...
        private WeakConccurentCollection<PmFrameBase> GetAllFramesFromFlows(FsUnidirectionalFlow upFlow,
            FsUnidirectionalFlow downFlow)
        {
            ...
            if (upFlow == null && downFlow != null)
                ...
                frames.AddRange(this.L7PDUs.SelectMany(p => p.FrameList).Concat(downFlow.NonDataFrames)
                    .Concat(upFlow.NonDataFrames));
            return frames;
        }

        private HashSet<PmCaptureBase> GetAllCapturesFromFlows(FsUnidirectionalFlow upFlow,
            FsUnidirectionalFlow downFlow)
        {
        ...
        [Required] public Guid L4ConversationRefId { get; private set; }
        ...
        [Required] public Guid L3ConversationRefId { get; private set; }
        ...
        public L7ConversationStatistics UpConversationStatistic
        {
            get
            {
                return this._upConversationStatistic ?? (this._upConversationStatistic =
                    this.ConversationFlowStatistics.FirstOrDefault(s => s.FlowDirection == DaRFlowDirection.up));
            }
            set
            {
        ...
        public L7ConversationStatistics DownConversationStatistic
        {
            get
            {
                return this._downConversationStatistic ?? (this._downConversationStatistic =
                    this.ConversationFlowStatistics.FirstOrDefault(s => s.FlowDirection == DaRFlowDirection.down));
            }
            set
            {
        ...
        public IEnumerable<PmFrameBase> VirtualFrames
        {
            get
            {
                return this.Frames.Where(f =>
                    f.GetType() == typeof(PmFrameVirtualBlank) || f.GetType() == typeof(PmFrameVirtual));
            }
        }
        ...
        [IgnoreAutoChangeNotification]
        public IEnumerable<L7PDU> DownFlowPDUs =>
            this.L7PDUs?.Where(l7PDU => l7PDU.FlowDirection == DaRFlowDirection.down);
        ...
        [NotMapped]
        public L7ConversationStatistics L7ConversationStatistics => this._l7ConversationStatistics ??
                                                                    (this._l7ConversationStatistics =
                                                                        new L7ConversationStatistics(
                                                                            this.UpConversationStatistic,
                                                                            this.DownConversationStatistic));
        ...
        [NotMapped] public IPEndPoint DestinationEndPoint => this.L4Conversation?.DestinationEndPoint;

        [NotMapped] public IPEndPoint SourceEndPoint => this.L4Conversation?.SourceEndPoint;
        ...
        {
            get { return this.Frames.Where(f => Equals(f.SrcAddress, this.DestinationEndPoint.Address)); }
        }
        ...
        public DateTime FirstSeen
        {
            get
            {
                return (DateTime) (this._firstSeen ?? (this._firstSeen = this.L7PDUs?.FirstOrDefault()?.FirstSeen ??
                                                                         this.Frames?.FirstOrDefault()?.TimeStamp ??
                                                                         DateTime.MinValue));
            }
        ...
        public DateTime LastSeen
        {
            get
            {
                return (DateTime) (this._lastSeen ?? (this._lastSeen = this.L7PDUs?.LastOrDefault()?.LastSeen ??
                                                                       this.Frames?.LastOrDefault()?.TimeStamp ??
                                                                       DateTime.MaxValue));
            }
        ...
        [IgnoreAutoChangeNotification] public virtual ICollection<PmFrameBase> Frames { get; protected set; }
        ...
        {
            get { return this.Frames.Where(f => Equals(f.SrcAddress, this.SourceEndPoint.Address)); }
        }
        ...
        public Boolean IsXyProtocolConversation(String applicationTag) =>
            this.ApplicationTags.Contains(applicationTag, StringComparer.OrdinalIgnoreCase);

        public Boolean IsXyProtocolConversation(String[] applicationTag)
        {
            return applicationTag.Any(appTag =>
                this.ApplicationTags.Contains(appTag, StringComparer.OrdinalIgnoreCase));
        }

        public IReadOnlyList<NBAR2TaxonomyProtocol> RecognizeApplicationProtocols(
            IApplicationRecognizer applicationRecognizer)
        {
        ...
        {
            return
                $"{this.ProtocolType}, {this.SourceEndPoint}-{this.DestinationEndPoint}, {this.FirstSeen}-{this.LastSeen}, {this.L3ProtocolType}, {this.L4ProtocolType}, {nameof(this.CipherSuite)}: {this.CipherSuite},  {nameof(this.AppTag)}: {this.AppTag}, {nameof(this.AppTagProvider)}: {this.AppTagProvider}{Environment.NewLine}{nameof(this.UpConversationStatistic)}: {this.UpConversationStatistic}{Environment.NewLine}{nameof(this.DownConversationStatistic)}: {this.DownConversationStatistic}{Environment.NewLine}{nameof(ConversationStats)}: {ConversationStats}";
        }

        private List<L7PDU> MergeUpDownPdus(IEnumerable<L7PDU> upFlowPdusEnumerable,
            IEnumerable<L7PDU> downFlowPDUseEnumerable)
        {
            List<L7PDU> l7Pdus;
            if (upFlowPdusEnumerable == null && downFlowPDUseEnumerable != null)
                l7Pdus = downFlowPDUseEnumerable.ToList();
            else if (downFlowPDUseEnumerable == null && upFlowPdusEnumerable != null)
                l7Pdus = upFlowPdusEnumerable.ToList();
            else
            {
        ...
        public IReadOnlyList<NBAR2TaxonomyProtocol> ApplicationProtocols
        {
            get
            {
                if (this._applicationProtocols == null)
                {
                    ...
                    this._applicationProtocols = this._applicationTags?.Where(tag => tag != string.Empty)
                        .Select(appTag => appRecognizer.GetNbar2TaxonomyProtocol(appTag)).ToList();
                }
                       ...
                       ?? (this._applicationTags =
                           (this.ApplicationProtocols != null && this.ApplicationProtocols.Any()
                               ? new PersistableJsonSerializableString(
                                   this.ApplicationProtocols.Select(proto => proto?.name))
                               : new PersistableJsonSerializableString()));
            }
        ...
        [NotMapped] public virtual IEnumerable<PmFrameBase> FramesNonData => this.Frames.Where(f => f.L7Pdu == null);
        ...
        public override Int32 GetHashCode() => (this.SourceEndPoint).GetHashCode() ^
                                               (this.DestinationEndPoint).GetHashCode() ^ this.FirstSeen.GetHashCode() ^
                                               this.LastSeen.GetHashCode();
